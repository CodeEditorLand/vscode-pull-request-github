...
export function CommentView(commentProps: Props) {
	const { isPRDescription, children, comment, headerInEditMode } =
		commentProps;
	...
	const { deleteComment, editComment, setDescription, pr } =
	...
	if (inEditMode) {
		return React.cloneElement(
			headerInEditMode ? <CommentBox for={comment} /> : <></>,
			{},
			[
				<EditComment
					id={id}
	...
	const ariaAnnouncement =
		(comment as CommentEvent | ReviewEvent).event === EventType.Commented ||
		(comment as CommentEvent | ReviewEvent).event === EventType.Reviewed
			? ariaAnnouncementForReview(comment as CommentEvent | ReviewEvent)
			: undefined;
	...
	return (
		<CommentBox
			...
			{ariaAnnouncement ? (
				<div role="alert" aria-label={ariaAnnouncement} />
			) : null}
			<div
				className="action-bar comment-actions"
				style={{ display: showActionBar ? "flex" : "none" }}>
				<button
					title="Quote reply"
				...
				{canDelete ? (
					<button
						...
						className="icon-button"
						onClick={() =>
							deleteComment({ id, pullRequestReviewId })
						}>
						{deleteIcon}
					</button>
				) : null}
...
function CommentBox({
	...
	const htmlUrl =
		"htmlUrl" in comment ? comment.htmlUrl : (comment as PullRequest).url;
	const isDraft =
		(comment as IComment).isDraft ??
		(isReviewEvent(comment) &&
			comment.state.toLocaleUpperCase() === "PENDING");
	const author =
		"user" in comment ? comment.user! : (comment as PullRequest).author!;
	const createdAt =
		"createdAt" in comment
			? comment.createdAt
			: (comment as ReviewEvent).submittedAt;

	return (
		<div
			className="comment-container comment review-comment"
				...
				<h3 className="review-comment-header">
					<Spaced>
						...
						{createdAt ? (
							<>
								{isReviewEvent(comment)
									? reviewDescriptor(comment.state)
									: "commented"}
								{nbsp}
...
function EditComment({ id, body, onCancel, onSave }: EditCommentProps) {
	...
	const draftComment = useRef<{ body: string; dirty: boolean }>({
		body,
		dirty: false,
	});
...
export const CommentBody = ({
	...
	const containsSuggestion =
		((body || bodyHTML)?.indexOf("```diff") ?? -1) > -1;
	const applyPatchButton =
		containsSuggestion && canApplyPatch && comment ? (
...
export function AddComment({
	...
	const { updatePR, requestChanges, approve, close, openOnGitHub, submit } =
		useContext(PullRequestContext);
	...
	let currentSelection: ReviewType =
		lastReviewType ??
		(currentUserReviewState === "APPROVED"
			? ReviewType.Approve
			: currentUserReviewState === "CHANGES_REQUESTED"
	...
	return (
		<form
			...
			onSubmit={() => submit(textareaRef.current?.value ?? "")}>
			<textarea
				id="comment-textarea"
				name="body"
				...
				<ContextDropdown
					...
					allOptions={() => {
						...
						if (availableActions.approve) {
							actions.push({
								label: availableActions[ReviewType.Approve]!,
								value: ReviewType.Approve,
								action: () => submitAction(ReviewType.Approve),
							});
						}
						if (availableActions.comment) {
							actions.push({
								label: availableActions[ReviewType.Comment]!,
								value: ReviewType.Comment,
								action: () => submitAction(ReviewType.Comment),
							});
						}
						if (availableActions.requestChanges) {
							actions.push({
								...
								action: () =>
									submitAction(ReviewType.RequestChanges),
							});
						}
						return actions;
					}}
...
const makeCommentMenuContext = (
	...
	pendingCommentText: string | undefined,
) => {
	...
	if (availableActions.requestChanges) {
		if (
			availableActions.requestChanges === COMMENT_METHODS.requestChanges
		) {
			createMenuContexts["github:reviewCommentRequestChanges"] = true;
		} else {
			createMenuContexts["github:reviewCommentRequestChangesOnDotCom"] =
				true;
		}
	}
...
export const AddCommentSimple = (pr: PullRequest) => {
	const { updatePR, requestChanges, approve, submit, openOnGitHub } =
		useContext(PullRequestContext);
	const [isBusy, setBusy] = useState(false);
	const textareaRef = useRef<HTMLTextAreaElement>();
	let currentSelection: ReviewType =
		pr.lastReviewType ??
		(pr.currentUserReviewState === "APPROVED"
			? ReviewType.Approve
			: pr.currentUserReviewState === "CHANGES_REQUESTED"
				? ReviewType.RequestChanges
				: ReviewType.Comment);
