...
const AutoMergeLabel = ({
	...
	if (busy) {
		return (
			<label
				htmlFor="automerge-checkbox"
				className="automerge-checkbox-label">
				Setting...
			</label>
		);
	} else {
...
export const AutoMerge = ({
	...
	updateState: (
		params: Partial<{ autoMerge: boolean; autoMergeMethod: MergeMethod }>,
	) => Promise<void>;
	baseHasMergeQueue: boolean;
	...
	const select: React.MutableRefObject<HTMLSelectElement> =
		React.useRef<HTMLSelectElement>() as React.MutableRefObject<HTMLSelectElement>;
			...
			<div className="automerge-checkbox-wrapper">
				<input
					...
					onChange={async () => {
						...
						await updateState({
							autoMerge: !autoMerge,
							autoMergeMethod: selectedMethod(),
						});
						setBusy(false);
					}}></input>
			</div>
...
export const QueuedToMerge = ({
	...
	switch (mergeQueueEntry.state) {
		...
		case MergeQueueState.Queued: {
			title = (
				<span className="merge-queue-pending">Queued to merge...</span>
			);
			if (mergeQueueEntry.position === 1) {
				message = (
					<span>
						This pull request is at the head of the{" "}
						<a href={mergeQueueEntry.url}>merge queue</a>.
					</span>
				);
			} else {
				message = (
					<span>
						This pull request is in the{" "}
						<a href={mergeQueueEntry.url}>merge queue</a>.
					</span>
				);
			}
		...
		case MergeQueueState.Locked: {
			title = (
				<span className="merge-queue-blocked">Merging is blocked</span>
			);
	...
	return (
		<div className="merge-queue-container">
