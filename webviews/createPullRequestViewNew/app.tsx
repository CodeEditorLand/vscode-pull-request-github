...
type CreateMethod =
	| "create-draft"
	| "create"
	| "create-automerge-squash"
	| "create-automerge-rebase"
	| "create-automerge-merge";

export const ChooseRemoteAndBranch = ({
	onClick,
	defaultRemote,
	defaultBranch,
	isBase,
	...
	onClick: (remote?: RemoteInfo, branch?: string) => Promise<void>;
	defaultRemote: RemoteInfo | undefined;
	...
	const defaultsLabel =
		defaultRemote && defaultBranch
			? `${remoteCount > 1 ? `${defaultRemote.owner}/` : ""}${defaultBranch}`
			: "\u2014";
	const title = isBase
		? "Base branch: " + defaultsLabel
		: "Branch to merge: " + defaultsLabel;
	...
	render(
		<Root>
			{(params: CreateParamsNew) => {
				...
				function createMethodLabel(
					isDraft?: boolean,
					autoMerge?: boolean,
					autoMergeMethod?: MergeMethod,
					baseHasMergeQueue?: boolean,
				): { value: CreateMethod; label: string } {
					...
					if (autoMerge && baseHasMergeQueue) {
						...
						value =
							`create-automerge-${autoMergeMethod}` as CreateMethod;
						const mergeMethodLabel =
							autoMergeMethod.charAt(0).toUpperCase() +
							autoMergeMethod.slice(1);
				...
				function updateTitle(title: string): void {
					if (params.validate) {
						ctx.updateState({
							pendingTitle: title,
							showTitleValidationError: !title,
						});
					} else {
				...
				const onCreateButton: React.MouseEventHandler<
					HTMLButtonElement
				> = (event) => {
					const selected = (event.target as HTMLButtonElement)
						.value as CreateMethod;
					let isDraft = false;
					let autoMerge = false;
				...
				function makeCreateMenuContext(createParams: CreateParamsNew) {
					...
					if (createParams.baseHasMergeQueue) {
						createMenuContexts[
							"github:createPrMenuMergeWhenReady"
						] = true;
					} else {
						if (
							createParams.allowAutoMerge &&
							createParams.mergeMethodsAvailability &&
							createParams.mergeMethodsAvailability["merge"]
						) {
							createMenuContexts["github:createPrMenuMerge"] =
								true;
						}
						if (
							createParams.allowAutoMerge &&
							createParams.mergeMethodsAvailability &&
							createParams.mergeMethodsAvailability["squash"]
						) {
							createMenuContexts["github:createPrMenuSquash"] =
								true;
						}
						if (
							createParams.allowAutoMerge &&
							createParams.mergeMethodsAvailability &&
							createParams.mergeMethodsAvailability["rebase"]
						) {
							createMenuContexts["github:createPrMenuRebase"] =
								true;
						}
					}
				...
				if (
					ctx.createParams.initializeWithGeneratedTitleAndDescription
				) {
					ctx.createParams.initializeWithGeneratedTitleAndDescription =
						false;
					generateTitle(true);
					...
					<div
						...
						<div className="group-branches">
							<div className="input-label base">
								...
								<ChooseRemoteAndBranch
									onClick={ctx.changeBaseRemoteAndBranch}
							...
							<div className="input-label merge">
								...
								{ctx.createParams.canModifyBranches ? (
									<ChooseRemoteAndBranch
										onClick={ctx.changeMergeRemoteAndBranch}
						...
						<div className="group-title">
							<input
								...
								className={
									params.showTitleValidationError
										? "input-error"
										: ""
								}
							...
							{ctx.createParams
								...
								isGeneratingTitle ? (
									<a
										title="Cancel"
										className={`title-action icon-button${isBusy || !ctx.initialized ? "disabled" : ""}`}
										onClick={ctx.cancelGenerateTitle}
										tabIndex={0}>
										{stopIcon}
									</a>
								) : (
							...
							<div
								...
								className={
									params.showTitleValidationError
										? "validation-error below-input-error"
										: "hidden"
								}>
								A title is required
							</div>
						...
						<div className="group-additions">
							{params.assignees && params.assignees.length > 0 ? (
								<div className="assignees">
									<span title="Assignees" aria-hidden="true">
										{assigneeIcon}
									</span>
									<ul
							...
							{params.reviewers && params.reviewers.length > 0 ? (
								<div className="reviewers">
									<span title="Reviewers" aria-hidden="true">
										{reviewerIcon}
									</span>
									<ul
							...
							{params.labels && params.labels.length > 0 ? (
								<div className="labels">
									<span title="Labels" aria-hidden="true">
										{labelIcon}
									</span>
									<ul
										aria-label="Labels"
							...
							{params.milestone ? (
								<div className="milestone">
									<span title="Milestone" aria-hidden="true">
										{milestoneIcon}
									</span>
									<ul
							...
							{params.projects && params.projects.length > 0 ? (
								<div className="projects">
									<span title="Projects" aria-hidden="true">
										{projectIcon}
									</span>
									<ul
										aria-label="Project"
						...
						<div
							className={
								params.validate && !!params.createError
									? "wrapper validation-error"
									: "hidden"
							}
							aria-live="assertive">
							<ErrorBoundary>{params.createError}</ErrorBoundary>
						</div>
						...
						<div className="group-actions">
							...
							<ContextDropdown
								optionsContext={() =>
									makeCreateMenuContext(params)
								}
								defaultAction={onCreateButton}
								defaultOptionLabel={() =>
									createMethodLabel(
										ctx.createParams.isDraft,
										ctx.createParams.autoMerge,
										ctx.createParams.autoMergeMethod,
										ctx.createParams.baseHasMergeQueue,
									).label
								}
								defaultOptionValue={() =>
									createMethodLabel(
