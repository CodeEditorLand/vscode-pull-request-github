...
export class PRContext {
	...
	public copyVscodeDevLink = () =>
		this.postMessage({ command: "pr.copy-vscodedevlink" });
	...
	public gotoChangesSinceReview = () =>
		this.postMessage({ command: "pr.gotoChangesSinceReview" });
	...
	public changeEmail = async (current: string) => {
		const newEmail = await this.postMessage({
			command: "pr.change-email",
			args: current,
		});
	...
	public merge = async (args: MergeArguments): Promise<MergeResult> => {
		const result: MergeResult = await this.postMessage({
			command: "pr.merge",
			args,
		});
		return result;
	...
	public openOnGitHub = () =>
		this.postMessage({ command: "pr.openOnGitHub" });
	...
	public readyForReview = (): Promise<ReadyForReview> =>
		this.postMessage({ command: "pr.readyForReview" });

	public addReviewers = () =>
		this.postMessage({ command: "pr.change-reviewers" });
	public changeProjects = (): Promise<ProjectItemsReply> =>
		this.postMessage({ command: "pr.change-projects" });
	public removeProject = (project: IProjectItem) =>
		this.postMessage({ command: "pr.remove-project", args: project });
	public addMilestone = () =>
		this.postMessage({ command: "pr.add-milestone" });
	public removeMilestone = () =>
		this.postMessage({ command: "pr.remove-milestone" });
	public addAssignees = () =>
		this.postMessage({ command: "pr.change-assignees" });
	public addAssigneeYourself = () =>
		this.postMessage({ command: "pr.add-assignee-yourself" });
	public addLabels = () => this.postMessage({ command: "pr.add-labels" });
	public create = () => this.postMessage({ command: "pr.open-create" });

	public deleteComment = async (args: {
		id: number;
		...
		if (!review.comments) {
			console.error(
				"No comments to delete for review:",
				pullRequestReviewId,
				review,
			);
			return;
	...
	public updateDraft = (id: number, body: string) => {
		...
		const pendingCommentDrafts =
			pullRequest.pendingCommentDrafts || Object.create(null);
	...
	public approve = async (body: string) =>
		this.appendReview(
			await this.postMessage({ command: "pr.approve", args: body }),
		);

	public submit = async (body: string) =>
		this.appendReview(
			await this.postMessage({ command: "pr.submit", args: body }),
		);
	...
	private appendReview({
		...
		const events = state.events.filter(
			(e) =>
				e.event !== EventType.Reviewed ||
				e.state.toLowerCase() !== "pending",
		);
		...
		state.events = [
			...state.events.filter((e) =>
				e.event === EventType.Reviewed ? e.state !== "PENDING" : e,
			),
			review,
		];
	...
	public async updateAutoMerge({
		...
		const response: { autoMerge: boolean; autoMergeMethod?: MergeMethod } =
			await this.postMessage({
				command: "pr.update-automerge",
				args: { autoMerge, autoMergeMethod },
			});
		const state = this.pr;
	...
	public updateBranch = async () => {
		const result: Partial<PullRequest> = await this.postMessage({
	...
	postMessage(message: any) {
		return (
			this._handler?.postMessage(message) ?? Promise.resolve(undefined)
		);
	}
	...
	handleMessage = (message: any) => {
		switch (message.command) {
			...
			case "pr.deleteBranch":
				...
				message.branchTypes &&
					message.branchTypes.map((branchType: string) => {
						if (branchType === "local") {
			...
			case "pr.scrollToPendingReview":
				const pendingReview =
					document.getElementById("pending-review") ??
			...
			case "pr.submitting-review":
				return this.updatePR({
					busy: true,
					lastReviewType: message.lastReviewType,
				});
