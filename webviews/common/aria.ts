...
export function ariaAnnouncementForReview(comment: ReviewEvent | CommentEvent) {
	...
	const veryRecentEvent =
		commentTime && Date.now() - new Date(commentTime).getTime() < 1000 * 60;
	const commentState =
		(comment as ReviewEvent).state ??
		((comment as CommentEvent).event === EventType.Commented
			? "COMMENTED"
			: undefined);
