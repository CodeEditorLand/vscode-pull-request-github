...
export class GitApiImpl implements API, IGit, vscode.Disposable {
	...
	registerTitleAndDescriptionProvider(
		...
		const disposable = {
			dispose: () =>
				this._titleAndDescriptionProviders.delete(registeredValue),
		};
	...
	getTitleAndDescriptionProvider(
		searchTerm?: string,
	): { title: string; provider: TitleAndDescriptionProvider } | undefined {
		if (!searchTerm) {
	...
	registerReviewerCommentsProvider(
		...
		const disposable = {
			dispose: () =>
				this._reviewerCommentsProviders.delete(registeredValue),
		};
		this._disposables.push(disposable);
		return disposable;
	}

	getReviewerCommentsProvider():
		| { title: string; provider: ReviewerCommentsProvider }
		| undefined {
		return this._reviewerCommentsProviders.size > 0
			? this._reviewerCommentsProviders.values().next().value
			: undefined;
	}
}
