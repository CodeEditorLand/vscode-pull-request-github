...
export class NotificationsManager
	extends Disposable
	implements vscode.TreeDataProvider<NotificationTreeDataItem>
{
	...
	get sortingMethod(): NotificationsSortMethod {
	...
	async getChildren(
		element?: unknown,
	): Promise<NotificationTreeDataItem[] | undefined> {
	...
	private _resolveNotificationTreeItem(
		element: NotificationTreeItem,
	): vscode.TreeItem {
		...
		if (
			...
			item.iconPath = model.isOpen
				? new vscode.ThemeIcon(
						"git-pull-request",
						new vscode.ThemeColor("pullRequests.open"),
					)
				: new vscode.ThemeIcon(
						"git-pull-request",
	...
	public async getNotifications(): Promise<
		INotificationTreeItems | undefined
	> {
		if (this._fetchNotifications) {
			// Get raw notifications
			const notificationsData =
				await this._notificationProvider.getNotifications(
					this._dateTime.toISOString(),
					this._pageCount,
				);
			...
			const notificationTreeItems = new Map<
				string,
				NotificationTreeItem
			>();
			await Promise.all(
				notificationsData.notifications.map(async (notification) => {
					const model =
						await this._notificationProvider.getNotificationModel(
							notification,
						);
		...
		if (this._sortingMethod === NotificationsSortMethod.Priority) {
			const notificationsWithoutPriority = Array.from(
	...
	public async markAsRead(notificationIdentifier: {
		...
		const notification = this._notifications.get(
			notificationIdentifier.notificationKey,
		);
