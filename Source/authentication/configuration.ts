...
export const HostHelper = class {
	public static async getApiHost(
		host: IHostConfiguration | vscode.Uri,
	): Promise<vscode.Uri> {
		...
		if (testEnv) {
			...
			const result = await vscode.window.showInformationMessage(
				vscode.l10n.t(
					"The 'GITHUB_TEST_SERVER' environment variable is set to '{0}'. Use this as the GitHub API endpoint?",
					testEnv,
				),
				{ modal: true },
				yes,
		...
		const hostUri: vscode.Uri =
			host instanceof vscode.Uri ? host : vscode.Uri.parse(host.host);
