...
export class GitHubManager {
	...
	private _knownServers: Map<string, GitHubServerType> = new Map([
		...
		...Array.from(GitHubManager._gheServers.keys()).map((key) => [
			key,
			GitHubServerType.Enterprise,
		]),
	] as [string, GitHubServerType][]);
	...
	public async isGitHub(host: vscode.Uri): Promise<GitHubServerType> {
		...
		if (
			...
			authority.match(/gist[.]github[.]com/)
		) {
			return GitHubServerType.None;
		}
		...
		const matchingKnownServer = Array.from(this._knownServers.keys()).find(
			(server) => authority.endsWith(server),
		);
		...
		if (
			...
			(!matchingKnownServer ||
				this._knownServers.get(matchingKnownServer) ===
