...
function ensurePR<TIssue extends Issue, TIssueModel extends IssueModel<TIssue>>(
	folderRepoManager: FolderRepositoryManager,
	pr?: TIssueModel,
): TIssueModel;
function ensurePR<TIssue extends Issue, TIssueModel extends IssueModel<TIssue>>(
	folderRepoManager: FolderRepositoryManager,
	pr?: PRNode | TIssueModel,
): TIssueModel {
...
export async function openDescription(
	...
	if (pullRequest instanceof PullRequestModel) {
		await PullRequestOverviewPanel.createOrShow(
			telemetry,
			folderManager.context.extensionUri,
			folderManager,
			pullRequest,
			undefined,
			preserveFocus,
...
export async function openPullRequestOnGitHub(
	e: PRNode | DescriptionNode | PullRequestModel | NotificationTreeItem,
	telemetry: ITelemetry,
) {
	if (e instanceof PRNode || e instanceof DescriptionNode) {
		...
		vscode.commands.executeCommand(
			"vscode.open",
			vscode.Uri.parse(e.model.html_url),
		);
	} else {
		vscode.commands.executeCommand(
			"vscode.open",
...
export function registerCommands(
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("review.suggestDiff", async (e) => {
			...
			const hasShownMessage = context.globalState.get(
				hasShownMessageKey,
				false,
			);
			if (!hasShownMessage) {
				...
				const result = await vscode.window.showInformationMessage(
					vscode.l10n.t(
						"You can now make suggestions from review comments, just like on GitHub.com. See the documentation for more details.",
					),
					{ modal: true },
					documentation,
				);
				if (result === documentation) {
					return vscode.env.openExternal(
						vscode.Uri.parse(
							"https://github.com/microsoft/vscode-pull-request-github/blob/main/documentation/suggestAChange.md",
						),
					);
				}
			}
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(
			...
			async (e: GitFileChangeNode) => {
				...
				if (folderManager) {
					const imageDataURI = await asTempStorageURI(
	...
	async function openDiffView(
		...
		if (fileChangeNode && !(fileChangeNode instanceof vscode.Uri)) {
			...
			const editor =
				fileChangeNode instanceof vscode.Uri
					...
					if (folderManager?.activePullRequest) {
						...
						const result =
							await vscode.window.showInformationMessage(
								vscode.l10n.t(
									"You already have a pull request for this branch. Do you want to push your changes to the remote branch?",
								),
								{ modal: true },
								push,
							);
	...
	context.subscriptions.push(
		...
		*/
				telemetry.sendTelemetryEvent("pr.checkout", {
					fromDescription: fromDescriptionPage.toString(),
				});
		...
		vscode.commands.registerCommand(
			...
			async () => {
					...
					(folderManager) => {
						githubRepository =
							folderManager.gitHubRepositories.find(
								(githubRepo) =>
									githubRepo.remote.remoteName ===
									prUriPropserties.remoteName,
							);
				...
				const prModel = await vscode.window.withProgress(
					...
					() =>
						folderManager.fetchById(
							githubRepository!,
							Number(prUriPropserties.prNumber),
						),
				);
				if (prModel && !isCheckingOutFromReadonlyFile) {
					isCheckingOutFromReadonlyFile = true;
					try {
						await ReviewManager.getReviewManagerForFolderManager(
							reviewsManager.reviewManagers,
							folderManager,
						)?.switch(prModel);
					} catch (e) {
						vscode.window.showErrorMessage(
							vscode.l10n.t(
								"Unable to check out pull request from read-only file: {0}",
	...
	context.subscriptions.push(
		...
		*/
				telemetry.sendTelemetryEvent("pr.exit", {
					fromDescription: fromDescriptionPage.toString(),
				});
		...
		vscode.commands.registerCommand(
			...
			async (argument: DescriptionNode | IssueModel | undefined) => {
				...
				await openDescription(
					...
					folderManager,
					!(argument instanceof DescriptionNode),
					!(argument instanceof RepositoryChangesNode),
					tree.notificationProvider,
				);
			},
		),
	...
	const unresolve = async (
		...
		*/
		...
		if (handler) {
			...
			if (focusReply) {
				thread.reveal(undefined, {
					focus: vscode.CommentThreadFocus.Reply,
				});
			}
		}
	};
	...
	const localUriFromReviewUri = (reviewUri: vscode.Uri) => {
		...
		return vscode.Uri.joinPath(vscode.Uri.file(rootPath), path).with({
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(
			...
			async (commentThread: GHPRCommentThread) => {
				/* __GDPR__
			"pr.diffOutdatedCommentWithHead" : {}
			*/
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(
			"pr.makeSuggestion",
			async (reply: CommentReply | GHPRComment | undefined) => {
				let potentialThread: GHPRCommentThread | undefined;
				if (reply === undefined) {
					potentialThread = findActiveHandler()?.commentController
						.activeCommentThread as vscode.CommentThread2 as
						| GHPRCommentThread
						| undefined;
				} else {
					potentialThread =
						reply instanceof GHPRComment
							? reply.parent
							: reply?.thread;
				}
				...
				const position = commentEditor.document.lineAt(
					commentEditor.selection.end.line,
				).range.end;
...
\`\`\``,
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(
			...
			(_value: vscode.Uri) => {
				const value =
					_value ?? vscode.window.activeTextEditor?.document.uri;
				if (!value) {
					return;
				}
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(
			...
			async (
				value:
					| { resourceStates: { resourceUri }[] }
					| { resourceUri: vscode.Uri },
				...additionalSelected: { resourceUri: vscode.Uri }[]
			) => {
				let resources: vscode.Uri[];
				if ("resourceStates" in value) {
					resources = value.resourceStates.map(
	...
	context.subscriptions.push(
		vscode.commands.registerDiffInformationCommand(
			...
			async (diffLines: vscode.LineChange[]) => {
				const tab = vscode.window.tabGroups.activeTabGroup.activeTab;
				...
				const selectedLines = diffLines.filter((line) => {
					return !!editorSelection.intersection(
				...
				return reviewManager?.createSuggestionFromChange(
					editor.document,
					selectedLines,
				);
			},
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("pr.setFileListLayoutAsTree", (_) => {
			vscode.workspace
				...
				.update(FILE_LIST_LAYOUT, "tree", true);
		}),
	);
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("pr.setFileListLayoutAsFlat", (_) => {
			vscode.workspace
				.getConfiguration(PR_SETTINGS_NAMESPACE)
				.update(FILE_LIST_LAYOUT, "flat", true);
		}),
	);
	...
	const findPrFromUri = (
		manager: FolderRepositoryManager | undefined,
		treeNode: vscode.Uri,
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("pr.checkoutByNumber", async () => {
			...
			for (const manager of reposManager.folderManagers) {
				const remotes = await manager.getActiveGitHubRemotes(
					await manager.getGitHubRemotes(),
				);
				const activeGitHubRepos = manager.gitHubRepositories.filter(
					(repo) =>
						remotes.find(
							(remote) =>
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(
			...
			async (fileChangeNode: GitFileChangeNode) => {
				...
				return vscode.commands.executeCommand(
					...
					parentURI,
					headURI,
					`${fileName} (Pull Request Compare Base with Head)`,
				);
			},
		),
	);
		...
		vscode.commands.registerCommand(
			...
			async (fileChangeNode: GitFileChangeNode) => {
				...
				return vscode.commands.executeCommand(
					...
					headURI,
					fileChangeNode.resourceUri,
					`${fileName} (Pull Request Compare Head with Local)`,
				);
			},
		),
	);
	...
	async function goToNextPrevDiff(diffs: vscode.LineChange[], next: boolean) {
		...
		for (
			...
			const index = next
				? i
				: reviewManager.reviewModel.localFileChanges.length - 1 - i;
			...
			if (
				...
				if (
					...
					await reviewManager.reviewModel.localFileChanges[
						nextIndex
						...
						const editor = vscode.window.visibleTextEditors.find(
							(editor) =>
								...
								reviewManager.reviewModel.localFileChanges[
									nextIndex
								].changeModel.filePath.toString(),
						);
						if (editor) {
							const diffs =
								await reviewManager.reviewModel.localFileChanges[
									nextIndex
								].changeModel.diffHunks();
							const diff = diffs[diffs.length - 1];
							const diffNewEndLine =
								diff.newLineNumber + diff.newLength;
							const practicalModifiedEndLineNumber =
								diffNewEndLine > diff.newLineNumber
									? diffNewEndLine
									: (diff.newLineNumber as number) + 1;
							const diffRange = new vscode.Range(
								diff.newLineNumber
									? diff.newLineNumber - 1
									: diff.newLineNumber,
								0,
								practicalModifiedEndLineNumber,
								0,
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("pr.refreshComments", async () => {
			for (const folderManager of reposManager.folderManagers) {
				for (const githubRepository of folderManager.gitHubRepositories) {
					for (const pullRequest of githubRepository.pullRequestModels) {
						if (
							pullRequest[1].isResolved() &&
							pullRequest[1].reviewThreadsCacheReady
						) {
