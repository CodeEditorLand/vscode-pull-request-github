...
import { getRootUriFromScmInputUri, isComment, issueMarkdown } from "./util";
...
export class IssueCompletionProvider implements vscode.CompletionItemProvider {
	...
	async provideCompletionItems(
			...
			for (const visibleEditor of vscode.window.visibleTextEditors) {
				const testFolderUri =
					vscode.workspace.workspaceFolders[0].uri.with({
						path: visibleEditor.document.uri.path,
					});
				const workspace =
					vscode.workspace.getWorkspaceFolder(testFolderUri);
				if (workspace) {
					uri = workspace.uri;
					break;
				}
		...
		for (const issueQuery of issueData) {
			...
			for (const issue of issuesOrMilestones.issues ?? []) {
				if (
					filterOwnerAndRepo &&
					((issue as IssueModel).remote.owner !==
						filterOwnerAndRepo.owner ||
						(issue as IssueModel).remote.repositoryName !==
							filterOwnerAndRepo.repo)
				) {
					continue;
				}
				completionItems.set(
	...
	private async completionItemFromIssue(
		...
		if (document.languageId === "markdown") {
			...
			if (
				document.uri.path.match(/git\/scm\d\/input/) &&
				typeof configuration === "string"
			) {
				item.insertText = await variableSubstitution(
					configuration,
					issue,
					repo,
				);
	...
	async resolveCompletionItem(
		...
		if (item instanceof IssueCompletionItem) {
			item.documentation = await issueMarkdown(
				item.issue,
				this.context,
