...
class IssueGroupNode {
	constructor(
		public readonly repoRootUri: vscode.Uri,
		public readonly queryLabel,
		public readonly isInFirstQuery: boolean,
		public readonly groupLevel: number,
		public readonly group: string,
		public readonly groupByOrder: QueryGroup[],
		public readonly issuesInGroup: IssueItem[],
	) {}
}
...
export class IssuesTreeData
	implements
		vscode.TreeDataProvider<
			FolderRepositoryManager | QueryNode | IssueGroupNode | IssueItem
		>
{
	...
	private getFolderRepoItem(
		element: FolderRepositoryManager,
	): vscode.TreeItem {
	...
	private getIssueTreeItem(element: IssueItem): vscode.TreeItem {
		...
		if (
			this.stateManager.currentIssue(element.uri)?.issue.number ===
			element.number
		) {
	...
	getChildren(
		...
		| FolderRepositoryManager[]
		| QueryNode[]
		| Promise<IssueItem[] | IssueGroupNode[]> {
		if (
			element === undefined &&
	...
	async resolveTreeItem(
		...
		if (element instanceof IssueModel) {
			item.tooltip = await issueMarkdown(
				element,
				this.context,
				this.manager,
			);
	...
	getStateChildren(): [] {
		if (
			this.manager.state === ReposManagerState.NeedsAuthentication ||
	...
	private getRepoChildren(
	...
	): QueryNode[] | Promise<IssueItem[] | IssueGroupNode[]> {
		...
		if (queryLabels.length === 1) {
			return this.getQueryNodeChildren(
				new QueryNode(
					folderManager.repository.rootUri,
					queryLabels[0],
					true,
	...
	private async getQueryNodeChildren(
		queryNode: QueryNode,
	): Promise<IssueItem[] | IssueGroupNode[]> {
		const issueCollection = this.stateManager.getIssueCollection(
		...
		const issueQueryResult = await issueCollection.get(
		...
		return this.getIssueGroupsForGroupIndex(
			...
			queryNode.queryLabel,
			queryNode.isFirst,
			issueQueryResult.groupBy,
			0,
			issueQueryResult.issues ?? [],
		);
	}

	private getIssueGroupsForGroupIndex(
		repoRootUri: vscode.Uri,
		queryLabel: string,
		isFirst: boolean,
		groupByOrder: QueryGroup[],
		indexInGroupByOrder: number,
		issues: IssueItem[],
	): IssueGroupNode[] | IssueItem[] {
		...
		if (
			(groupByValue !== "milestone" && groupByValue !== "repository") ||
			groupByOrder.findIndex((groupBy) => groupBy === groupByValue) !==
				indexInGroupByOrder
		) {
			return this.getIssueGroupsForGroupIndex(
				repoRootUri,
	...
	private async getIssueGroupChildren(
		issueGroupNode: IssueGroupNode,
	): Promise<IssueItem[] | IssueGroupNode[]> {
		return this.getIssueGroupsForGroupIndex(
			issueGroupNode.repoRootUri,
			issueGroupNode.queryLabel,
			issueGroupNode.isInFirstQuery,
			issueGroupNode.groupByOrder,
			issueGroupNode.groupLevel + 1,
			issueGroupNode.issuesInGroup,
		);
	}
...
export function updateExpandedQueries(
	...
	if (id) {
		const expandedQueries = new Set<string>(
			context.workspaceState.get(EXPANDED_ISSUES_STATE, []) as string[],
		);
...
function getQueryExpandState(
	context: vscode.ExtensionContext,
	element: FolderRepositoryManager | QueryNode | IssueGroupNode,
	defaultState: vscode.TreeItemCollapsibleState = vscode
