...
export class IssueFeatureRegistrar implements vscode.Disposable {
	...
	async initialize() {
		...
		this.context.subscriptions.push(
			new ShareProviderManager(this.manager, this.gitAPI),
		);
		this.context.subscriptions.push(
			vscode.commands.registerCommand(
				"issue.openIssue",
	...
	private async createTodoIssueBody(
		...
		let contents = "";
		if (newIssue) {
			const repository = getRepositoryForFile(
				this.gitAPI,
				newIssue.document.uri,
			);
			const changeAffectingFile =
				repository?.state.workingTreeChanges.find(
					(value) =>
						value.uri.toString() ===
	...
	private async makeNewIssueFile(
		...
		const bodyPath = vscode.Uri.parse(
			`${NEW_ISSUE_SCHEME}:/${NEW_ISSUE_FILE}${query}`,
		);
		if (
			vscode.window.visibleTextEditors.filter(
				(visibleEditor) =>
					visibleEditor.document.uri.scheme === NEW_ISSUE_SCHEME,
			).length > 0
		) {
			return;
		}
		...
		const assigneeLine = `${ASSIGNEES} ${
			assignees && assignees.length > 0
				? assignees.map((value) => "@" + value).join(", ") + " "
				: ""
		}`;
		...
		const text =
			cached && cached !== ""
				? cached
				: `${title ?? vscode.l10n.t("Issue Title")}\n
${assigneeLine}
${labelLine}
${milestoneLine}
${projectsLine}\n
