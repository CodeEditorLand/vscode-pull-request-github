...
export class IssueFeatureRegistrar extends Disposable {
		...
		this._register(
			vscode.commands.registerCommand(
				...
				(issue: any) => {
					if (
						!(
							issue instanceof IssueModel ||
							issue instanceof PRNode
						)
					) {
						return;
					}
		...
		this._stateManager.tryInitializeAndWait().then(() => {
			...
			this._register(
				vscode.languages.registerCodeActionsProvider(
					"*",
					new IssueTodoProvider(this.context),
					{
	...
	async createIssue() {
		...
		const remoteName =
			folderManager.repository.state.HEAD?.upstream?.remote;
		...
		if (!remote) {
			const potentialRemotes =
				folderManager.repository.state.remotes.filter(
			...
			const choice = await vscode.window.showQuickPick(choices, {
				placeHolder: vscode.l10n.t(
					"Select a remote to file this issue to",
				),
			});
			if (!choice) {
				return;
	...
	async doStartWorking(
		...
		if (remoteNameResult.needsFork) {
			if (
				(await repoManager.tryOfferToFork(githubRepository)) ===
				undefined
			) {
				return;
			}
	...
	async startWorking(issue: any) {
		if (issue instanceof IssueModel) {
			...
			const folderManager = this.manager.folderManagers.find(
				(folderManager) =>
	...
	async stopWorking(issueModel: any) {
		...
		if (
			...
			this._stateManager.currentIssue(folderManager.repository.rootUri)
				?.issue.number === issueModel.number
		) {
			await this._stateManager.setCurrentIssue(
				folderManager,
				undefined,
		...
		if (newIssue) {
			...
			const changeAffectingFile =
				folderRepoManager?.repository?.state.workingTreeChanges.find(
					(value) =>
						value.uri.toString() ===
						newIssue.document.uri.toString(),
				);
	...
	private async makeNewIssueFile(
		...
		const repoRef = folderManager.findRepo(
			(githubRepo) =>
				githubRepo.remote.remoteName === options?.remote?.name,
		)?.remote.gitProtocol;
...
<!-- ${vscode.l10n.t('Edit the body of your new issue then click the ✓ \"Create Issue\" button in the top right of the editor. The first line will be the issue title. Assignees and Labels follow after a blank line. Leave an empty line before beginning the body of the issue.')} -->`;
		...
		const editorChangeDisposable =
			vscode.window.onDidChangeActiveTextEditor((textEditor) => {
				if (textEditor?.document.uri.scheme === Schemes.NewIssue) {
					...
					const assigneeFullLine =
						textEditor.document.lineAt(assigneeLineNum);
					...
					const labelFullLine =
						textEditor.document.lineAt(labelLineNum);
					if (labelFullLine.text.startsWith(LABELS)) {
						textEditor.setDecorations(labelsDecoration, [
							new vscode.Range(
								...
								new vscode.Position(
									labelLineNum,
									labelFullLine.text.length,
								),
							),
						]);
					}
					const projectsFullLine =
						textEditor.document.lineAt(projectsLineNum);
				...
				if (issue) {
					...
					if (
						...
						const insertText: string =
							vscode.workspace
								.getConfiguration(ISSUES_SETTINGS_NAMESPACE)
								.get(CREATE_INSERT_FORMAT, "number") ===
							"number"
								? `#${issue.number}`
								: issue.html_url;
						edit.insert(
	...
	private async getPermalinkWithError(
		...
		const link = await createGithubPermalink(
			repositoriesManager,
			this.gitAPI,
			includeRange,
			includeFile,
	...
	async copyPermalink(
		...
		if (link.permalink) {
			const contextualizedLink =
				contextualizeLink && link.originalFile
