...
abstract class AbstractShareProvider
	implements vscode.Disposable, vscode.ShareProvider
{
	private disposables: vscode.Disposable[] = [];
	...
	private async initialize() {
		...
		this.disposables.push(
			this.repositoryManager.onDidLoadAnyRepositories(async () => {
				if (
					(await this.hasGitHubRepositories()) &&
					this.shouldRegister()
				) {
					this.register();
				}
	...
	private register() {
		...
		this.shareProviderRegistrations = supportedSchemes.map((scheme) =>
			vscode.window.registerShareProvider({ scheme }, this),
		);
	}
	...
	public async provideShare(
		item: vscode.ShareableItem,
	): Promise<vscode.Uri | string | undefined> {
		// Get the blob
		const folderManager = this.repositoryManager.getManagerForFile(
			item.resourceUri,
		);
		if (!folderManager) {
			throw new Error(
		...
		if (!remote || !remote.fetchUrl) {
			throw new Error(
				vscode.l10n.t("The selection may not exist on any remote."),
			);
		}
		...
		const path = encodeURIComponentExceptSlashes(
		...
		return vscode.Uri.parse(
			[
				origin,
				"/",
				getOwnerAndRepo(this.repositoryManager, repository, {
					...remote,
...
export class GitHubDevShareProvider
	extends AbstractShareProvider
	implements vscode.ShareProvider
{
	constructor(repositoryManager: RepositoriesManager, gitApi: GitApiImpl) {
		super(
			repositoryManager,
			gitApi,
			"githubDevLink",
			vscode.l10n.t("Copy github.dev Link"),
			10,
			"github.dev",
		);
	}
...
export class GitHubPermalinkShareProvider
	extends AbstractShareProvider
	implements vscode.ShareProvider
{
	constructor(
		...
		if (!commitHash) {
			...
			try {
				...
				if (
					...
					commit = await repository.getCommit(
						repository.state.HEAD.commit,
					);
				}
				if (!commit) {
	...
	protected async getUpstream(
		...
		return getBestPossibleUpstream(
			this.repositoryManager,
			repository,
			(await repository.getCommit(commit)).hash,
		);
	}
}
...
export class GitHubPermalinkAsMarkdownShareProvider extends GitHubPermalinkShareProvider {
	...
	constructor(repositoryManager: RepositoriesManager, gitApi: GitApiImpl) {
		super(
			repositoryManager,
			gitApi,
			"githubComPermalinkAsMarkdown",
			vscode.l10n.t("Copy GitHub Permalink as Markdown"),
			12,
		);
	}
	...
	async provideShare(
		...
		Logger.trace(
			"providing permalink markdown share",
			GitHubPermalinkAsMarkdownShareProvider.ID,
		);
	...
	private async getMarkdownLinkText(
		item: vscode.ShareableItem,
		...
		if (item.selection) {
			...
			const editorSelection =
				item.selection.start === item.selection.end
					? item.selection
					: new vscode.Range(
							item.selection.start,
							new vscode.Position(
								item.selection.start.line + 1,
								0,
							),
						);
...
export class GitHubHeadLinkShareProvider
	extends AbstractShareProvider
	implements vscode.ShareProvider
{
	constructor(repositoryManager: RepositoriesManager, gitApi: GitApiImpl) {
		super(
			...
			gitApi,
			"githubComHeadLink",
			vscode.l10n.t("Copy GitHub HEAD Link"),
			13,
		);
	}
