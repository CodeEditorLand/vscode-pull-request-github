...
export class ChatParticipant extends Disposable {
	constructor(
		...
		private readonly state: ChatParticipantState,
	) {
		super();

		const ghprChatParticipant = this._register(
	...
	async handleParticipantRequest(
		...
		const allTools = vscode.lm.tools.map(
			...
			for await (const part of response.stream) {
				if (part instanceof vscode.LanguageModelTextPart) {
					...
					const tool = vscode.lm.tools.find(
						(tool) => tool.name === part.name,
					);

					if (!tool) {
						throw new Error(
							"Got invalid tool choice: " + part.name,
						);
					}
				...
				for (const toolCall of toolCalls) {
					...
					for (let i = 0; i < toolCallResult.content.length; i++) {
						...
						if (!(part instanceof vscode.LanguageModelTextPart)) {
							...
							result = new vscode.LanguageModelToolResultPart(
								toolCall.call.callId,
