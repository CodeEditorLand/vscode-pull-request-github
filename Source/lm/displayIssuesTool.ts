...
export class DisplayIssuesTool extends ToolBase<DisplayIssuesParameters> {
	...
	async prepareToolInvocation(
		_options: vscode.LanguageModelToolInvocationPrepareOptions<DisplayIssuesParameters>,
	): Promise<vscode.PreparedToolInvocation> {
		return {
			invocationMessage: vscode.l10n.t(
				"Generating markdown table of issues...",
			),
		};
	}

	async invoke(
		_options: vscode.LanguageModelToolInvocationOptions<DisplayIssuesParameters>,
		token: vscode.CancellationToken,
		...
		if (issueItems.length === 0) {
			return {
				...
				"text/markdown": vscode.l10n.t(
					"No issues found. Please try another query.",
				),
			};
		}
		Logger.debug(
		...
		return {
			...
			[MimeTypes.textDisplay]: vscode.l10n.t(
				"Here's a markdown table of the first 10 issues.\n\n{0}\n\n",
				issues.value,
			),
		};
	}
