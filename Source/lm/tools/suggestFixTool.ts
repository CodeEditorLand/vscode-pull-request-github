...
export class SuggestFixTool extends RepoToolBase<IssueToolParameters> {
	...
	async prepareInvocation(
		options: vscode.LanguageModelToolInvocationPrepareOptions<IssueToolParameters>,
	): Promise<vscode.PreparedToolInvocation> {
		return {
			invocationMessage: options.input.issueNumber
				? vscode.l10n.t(
						"Suggesting a fix for issue #{0}",
						options.input.issueNumber,
					)
				: vscode.l10n.t("Suggesting a fix for the issue"),
		};
	}

	async invoke(
		options: vscode.LanguageModelToolInvocationOptions<IssueToolParameters>,
		...
		if (!folderManager) {
			throw new Error(
				`No folder manager found for ${repo.owner}/${repo.name}. Make sure to have the repository open.`,
			);
		}
		...
		if (!issue) {
			throw new Error(
				`No issue found for ${repo.owner}/${repo.name}/${options.input.issueNumber}. Make sure the issue exists.`,
			);
		}
		...
		result.comments.forEach((comment, index) => {
			messages.push(
				vscode.LanguageModelChatMessage.User(
					`Comment ${index}: ${comment.body}`,
				),
			);
		});

		const copilotCodebaseResult = await vscode.lm.invokeTool(
			"copilot_codebase",
			{
		...
		if (plainTextResult instanceof vscode.LanguageModelTextPart) {
			messages.push(
				vscode.LanguageModelChatMessage.User(
					`Below is some potential relevant workspace context to the issue. The user cannot see this result, so you should explain it to the user if referencing it in your answer.`,
				),
			);
