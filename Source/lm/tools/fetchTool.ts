...
export class FetchTool extends RepoToolBase<FetchToolParameters> {
	async invoke(
		options: vscode.LanguageModelToolInvocationOptions<FetchToolParameters>,
		_token: vscode.CancellationToken,
	): Promise<vscode.LanguageModelToolResult | undefined> {
		const { owner, name, folderManager } = await this.getRepoInfo(options);

		const issueOrPullRequest = await this._fetchIssueOrPR(
			options,
			folderManager,
			owner,
			name,
	...
	async prepareToolInvocation(
		options: vscode.LanguageModelToolInvocationPrepareOptions<FetchToolParameters>,
	): Promise<vscode.PreparedToolInvocation> {
		return {
			invocationMessage: options.parameters.issueNumber
				? vscode.l10n.t(
						"Fetching item #{0} from GitHub...",
						options.parameters.issueNumber,
					)
				: vscode.l10n.t("Fetching item from GitHub..."),
		};
	}

	private async _fetchIssueOrPR(
		options: vscode.LanguageModelToolInvocationOptions<FetchToolParameters>,
		folderManager: FolderRepositoryManager,
		owner: string,
		name: string,
	): Promise<PullRequestModel | IssueModel> {
		...
		if (!issueOrPullRequest) {
			issueOrPullRequest = await folderManager.resolvePullRequest(
				owner,
				name,
				options.parameters.issueNumber,
			);
