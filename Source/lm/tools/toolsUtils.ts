...
export abstract class RepoToolBase<T> extends ToolBase<T> {
	...
	protected async getRepoInfo(options: {
		...
		if (
			!folderManager &&
			this.repositoriesManager.folderManagers.length > 0
		) {
			folderManager = this.repositoriesManager.folderManagers[0];

			if (owner && name) {
				await folderManager.createGitHubRepositoryFromOwnerName(
					owner,
					name,
				);
			} else {
	...
	protected getGitHub(): GitHub | undefined {
		...
		if (
			this.credentialStore.isAuthenticated(
