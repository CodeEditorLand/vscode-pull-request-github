...
export namespace CommentReply {
	export function is(commentReply: any): commentReply is CommentReply {
		return (
			commentReply &&
			commentReply.thread &&
			commentReply.text !== undefined
		);
	}
}

const commentHandlers = new Map<
	string,
	{ handler: CommentHandler; repoRootUri: string }
>();
...
export function registerCommentHandler(
	...
	commentHandlers.set(key, {
		handler: commentHandler,
		repoRootUri: repository.rootUri.toString(),
	});
}
...
export function resolveCommentHandler(
	commentThread: GHPRCommentThread,
