...
async function getItems<T extends IAccount | ITeam | ISuggestedReviewer>(
	context: vscode.ExtensionContext,
	skipList: Set<string>,
	users: T[],
	picked: boolean,
	tooManyAssignable: boolean = false,
): Promise<(vscode.QuickPickItem & { user?: T })[]> {
	...
	const avatars = await DataUri.avatarCirclesAsImageDataUris(
		context,
		filteredUsers,
		16,
		16,
	...
	for (let i = 0; i < filteredUsers.length; i++) {
		...
		if (isSuggestedReviewer(user)) {
			detail =
				user.isAuthor && user.isCommenter
					? vscode.l10n.t(
...
export async function getAssigneesQuickPickItems(
	folderRepositoryManager: FolderRepositoryManager,
	gitHubRepository: GitHubRepository | undefined,
	remoteName: string,
	alreadyAssigned: IAccount[],
	item?: PullRequestModel,
	assignYourself?: boolean,
	...
	const [allAssignableUsers, participantsAndViewer] = await Promise.all([
		...
		item
			? folderRepositoryManager.getPullRequestParticipants(
	...
	const assigneePromises: Promise<
		(vscode.QuickPickItem & { user?: IAccount })[]
	>[] = [];
	...
	if (
		viewer &&
		!skipList.has(viewer.login) &&
		assignableUsers.findIndex(
			(assignableUser: IAccount) => assignableUser.login === viewer.login,
		) !== -1
	) {
		assigneePromises.push(
			getItems<IAccount>(
	...
	if (assignYourself) {
		const currentUser =
			viewer ??
			(await folderRepositoryManager.getCurrentUser(gitHubRepository));
		if (assignees.length !== 0) {
			assignees.unshift({
				kind: vscode.QuickPickItemKind.Separator,
...
async function getReviewersQuickPickItems(
	folderRepositoryManager: FolderRepositoryManager,
	remoteName: string,
	...
	const allAssignableUsers =
		await folderRepositoryManager.getAssignableUsers();
	const allTeamReviewers = isInOrganization
		? await folderRepositoryManager.getTeamReviewers(refreshKind)
		: [];
	...
	// used to track logins that shouldn't be added to pick list
	// e.g. author, existing and already added reviewers
	const skipList: Set<string> = new Set([author.login]);

	const reviewersPromises: Promise<
		(vscode.QuickPickItem & { reviewer?: IAccount | ITeam })[]
	>[] = [];
	...
	if (existingReviewers.length) {
		reviewersPromises.push(
			getItems<IAccount | ITeam>(
				folderRepositoryManager.context,
				skipList,
				existingReviewers.map((reviewer) => reviewer.reviewer),
				true,
			),
		);
	}
...
export async function reviewersQuickPick(
	folderRepositoryManager: FolderRepositoryManager,
	remoteName: string,
	isInOrganization: boolean,
	teamsCount: number,
...
> {
		...
		quickPick.buttons = [
			{
				...
				tooltip: vscode.l10n.t("Show or refresh team reviewers"),
			},
		];
	}
	quickPick.show();
	const updateItems = async (refreshKind: TeamReviewerRefreshKind) => {
		const slowWarning = setTimeout(() => {
			quickPick.placeholder = vscode.l10n.t(
				"Getting team reviewers can take several minutes. Results will be cached.",
			);
		}, 3000);
	...
	await updateItems(
		teamsCount !== 0 && teamsCount <= quickMaxTeamReviewers
...
function isProjectQuickPickItem(
	x: vscode.QuickPickItem | ProjectQuickPickItem,
): x is ProjectQuickPickItem {
	return (
		!!(x as ProjectQuickPickItem).id &&
		!!(x as ProjectQuickPickItem).project
	);
}

export async function getProjectFromQuickPick(
	folderRepoManager: FolderRepositoryManager,
	githubRepository: GitHubRepository,
	currentProjects: IProject[] | undefined,
	callback: (projects: IProject[]) => Promise<void>,
): Promise<void> {
	try {
		let selectedItems: vscode.QuickPickItem[] = [];
		async function getProjectOptions(): Promise<
			(ProjectQuickPickItem | vscode.QuickPickItem)[]
		> {
			const projects =
				await folderRepoManager.getAllProjects(githubRepository);
			if (!projects || !projects.length) {
				return [
					{
						label: vscode.l10n.t(
							"No projects created for this repository.",
						),
					},
				];
			}
			...
			const projectItems: (
				...
				if (
					currentProjects &&
					currentProjects.find((project) => project.id === result.id)
				) {
		...
		quickPick.onDidAccept(async () => {
			quickPick.hide();
			const projectsToAdd = quickPick.selectedItems
				.map((item) =>
					isProjectQuickPickItem(item) ? item.project : undefined,
				)
		...
		vscode.window.showErrorMessage(
			`Failed to add project: ${formatError(e)}`,
		);
	}
}
...
function isMilestoneQuickPickItem(
	x: vscode.QuickPickItem | MilestoneQuickPickItem,
): x is MilestoneQuickPickItem {
	return (
		!!(x as MilestoneQuickPickItem).id &&
		!!(x as MilestoneQuickPickItem).milestone
	);
}

export async function getMilestoneFromQuickPick(
	folderRepositoryManager: FolderRepositoryManager,
	githubRepository: GitHubRepository,
	currentMilestone: IMilestone | undefined,
	callback: (milestone: IMilestone | undefined) => Promise<void>,
): Promise<void> {
	try {
		...
		async function getMilestoneOptions(): Promise<
			(MilestoneQuickPickItem | vscode.QuickPickItem)[]
		> {
			...
			if (!milestones || !milestones.length) {
				return [
					{
						label: vscode.l10n.t(
							"No milestones created for this repository.",
						),
					},
				];
			}
			...
			const milestonesItems: (
				| MilestoneQuickPickItem
				| vscode.QuickPickItem
			)[] = milestones.map((result) => {
				const item = {
		...
		quickPick.onDidTriggerButton((_) => {
			...
			inputBox.onDidAccept(async () => {
				...
				if (inputBox.value.length > 255) {
					vscode.window.showErrorMessage(
						vscode.l10n.t(
							`Failed to create milestone: The title can contain a maximum of 255 characters`,
						),
					);
					return;
				}
				...
				for (const existingMilestone of quickPick.items) {
					if (existingMilestone.label === inputBox.value) {
						vscode.window.showErrorMessage(
							vscode.l10n.t(
								"Failed to create milestone: The milestone '{0}' already exists",
								inputBox.value,
							),
						);
						return;
				...
				try {
					...
					if (
						...
						vscode.window.showErrorMessage(
							vscode.l10n.t(
								"Failed to create milestone: The milestone already exists and might be closed",
							),
						);
					} else {
						vscode.window.showErrorMessage(
							`Failed to create milestone: ${formatError(e)}`,
						);
					}
				}
			});
		...
		quickPick.onDidAccept(async () => {
			...
			if (milestoneToAdd && isMilestoneQuickPickItem(milestoneToAdd)) {
				...
				milestoneToAdd &&
				milestoneToAdd === removeMilestoneItem
			) {
		...
		vscode.window.showErrorMessage(
			`Failed to add milestone: ${formatError(e)}`,
		);
	}
}
...
export async function getLabelOptions(
	...
	const newLabels = await folderRepoManager.getLabels(undefined, {
		owner: baseOwner,
		repo: repositoryName,
	});
	...
	const labelPicks = newLabels.map((label) => {
		return {
			...
			iconPath: DataUri.asImageDataURI(
				Buffer.from(
...
export async function pickEmail(
	githubRepository: GitHubRepository,
	current: string,
	...
	const result = await vscode.window.showQuickPick(getEmails(), {
		canPickMany: false,
		title: vscode.l10n.t("Choose an email"),
	});
