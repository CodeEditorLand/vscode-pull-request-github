...
export namespace PullRequestView {
	export async function deleteBranch(
	...
	): Promise<{ isReply: boolean; message: any }> {
		const branchInfo =
			await folderRepositoryManager.getBranchNameForPullRequest(item);
		const actions: (vscode.QuickPickItem & {
			type: "upstream" | "local" | "remote" | "suspend";
		})[] = [];
		const defaultBranch =
			await folderRepositoryManager.getPullRequestRepositoryDefaultBranch(
				item,
			);
			...
			if (!isDefaultBranch && !item.isRemoteHeadDeleted) {
				actions.push({
					label: vscode.l10n.t(
						...
						`${item.remote.remoteName}/${branchHeadRef}`,
					),
		...
		if (branchInfo) {
			...
			if (
				branchInfo.remote &&
				branchInfo.createdForPullRequest &&
				!branchInfo.remoteInUse
			) {
				actions.push({
					label: vscode.l10n.t(
						"Delete remote {0}, which is no longer used by any other branch",
						branchInfo.remote,
					),
					type: "remote",
		...
		if (!actions.length) {
			vscode.window.showWarningMessage(
				vscode.l10n.t(
					"There is no longer an upstream or local branch for Pull Request #{0}",
					item.number,
				),
			);
			return {
		...
		if (selectedActions) {
			const isBranchActive = item.equals(
			...
			const promises = selectedActions.map(async (action) => {
				switch (action.type) {
					...
					case "local":
						if (isBranchActive) {
							if (
								folderRepositoryManager.repository.state
					...
					case "remote":
						deletedBranchTypes.push(action.type);
						return folderRepositoryManager.repository.removeRemote(
							branchInfo!.remote!,
						);
					case "suspend":
						deletedBranchTypes.push(action.type);
						return vscode.commands.executeCommand(
							"github.codespaces.disconnectSuspend",
						);
				}
			});
