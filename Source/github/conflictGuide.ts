...
export class ConflictModel implements vscode.Disposable {
	...
	private async listenForCommit() {
		...
		const result = await new Promise<boolean>((resolve) => {
			...
			localDisposable = this._repository.state.onDidChange(() => {
				if (
					this._repository.state.HEAD?.commit !== startingCommit &&
					this._repository.state.indexChanges.length === 0 &&
					this._repository.state.mergeChanges.length === 0
				) {
					resolve(true);
				}
			});
	...
	public static async begin(
		...
		into: string,
		push: boolean,
	): Promise<ConflictModel | undefined> {
...
class ConflictNotification implements vscode.Disposable {
	...
	constructor(
		private readonly _conflictModel: ConflictModel,
		private readonly _repository: Repository,
	) {
		vscode.window
			.withProgress(
				...
				async (progress, token) => {
					const report = (increment: number) => {
						progress.report({
							message: vscode.l10n.t(
								"Use the Source Control view to resolve conflicts, {0} of {0} remaining",
								this._conflictModel.remainingConflicts.length,
								this._conflictModel.startingConflictsCount,
							),
							increment,
						});
					};
					...
					return new Promise<boolean>((resolve) => {
						this._disposables.push(
							this._conflictModel.onConflictCountChanged(
								(conflictsChangedBy) => {
									const increment =
										conflictsChangedBy *
										(100 /
									...
									report(increment);
									if (
										this._conflictModel.remainingConflicts
											.length === 0
									) {
										resolve(true);
									}
			...
			.then(async (result) => {
				if (result) {
					...
					if (this._conflictModel.push) {
						message = vscode.l10n.t(
							"All conflicts resolved. Commit and push the resolution to continue.",
						);
					} else {
						message = vscode.l10n.t(
