...
async function createConflictResolutionModel(
	pullRequest: PullRequestModel,
): Promise<ConflictResolutionModel | undefined> {
	...
	const repositoryName = (await pullRequest.githubRepository.ensure()).remote
		.repositoryName;

	const potentialMergeConflicts: Conflict[] = [];

	if (pullRequest.item.mergeable === PullRequestMergeability.Conflict) {
		const mergeBaseIntoPrCompareData =
			await pullRequest.compareBaseBranchForMerge(
				prHeadOwner,
				prHeadRef,
				prBaseOwner,
				baseCommitSha,
		...
		if (
			pullRequest.item.mergeable === PullRequestMergeability.Conflict &&
			mergeBaseIntoPrCompareData.length >= 300
		) {
			// API limitation: it only returns the first 300 files
			return undefined;
		}

		const previousFilenames: Map<string, SlimFileChange | InMemFileChange> =
			new Map();
		...
		for (const mergeFile of mergeBaseIntoPrCompareData) {
			const fileChange =
				pullRequest.fileChanges.get(mergeFile.filename) ??
				previousFilenames.get(mergeFile.filename);

			if (
				fileChange &&
				(knownConflicts.size === 0 ||
					knownConflicts.has(fileChange.fileName))
			) {
...
export class DetachedHeadError extends Error {
	...
	override get message() {
		return vscode.l10n.t(
			"{0} has a detached HEAD (create a branch first",
			this.repository.rootUri.toString(),
		);
	}
}
...
export class BadUpstreamError extends Error {
	constructor(
		...
		public readonly upstreamRef: UpstreamRef,
	...
	override get message() {
		...
		return vscode.l10n.t(
			"The upstream ref {0} for branch {1} {2}.",
			`${remote}/${name}`,
			branchName,
			problem,
		);
...
export class FolderRepositoryManager extends Disposable {
	...
	constructor(
		...
		this._register(
			_credentialStore.onDidInitialize(() => this.updateRepositories()),
		);
	...
	public async computeAllUnknownRemotes(): Promise<Remote[]> {
		...
		const serverTypes = await Promise.all(
			potentialRemotes.map((remote) =>
				this._githubManager.isGitHub(
					remote.gitProtocol.normalizeUri()!,
				),
			),
		).catch((e) => {
	...
	public async getActiveGitHubRemotes(
		allGitHubRemotes: GitHubRemote[],
	): Promise<GitHubRemote[]> {
		const remotesSetting = vscode.workspace
		...
		if (missingRemotes.length === remotesSetting.length) {
			Logger.warn(
				`No remotes found. The following remotes are missing: ${missingRemotes.join(", ")}`,
			);
		} else {
			Logger.debug(
				`Not all remotes found. The following remotes are missing: ${missingRemotes.join(", ")}`,
				this.id,
			);
		}

		Logger.debug(
			`Displaying configured remotes: ${remotesSetting.join(", ")}`,
			this.id,
		);

		return remotesSetting
			.map((remote) =>
				allGitHubRemotes.find((repo) => repo.remoteName === remote),
			)
			.filter(
				(repo: GitHubRemote | undefined): repo is GitHubRemote =>
					!!repo &&
					!this._sessionIgnoredRemoteNames.has(repo.remoteName),
			);
	}
	...
	private async getActiveRemotes(): Promise<GitHubRemote[]> {
		...
		const activeRemotes = await this.getActiveGitHubRemotes(
			this._allGitHubRemotes,
		);
		...
		if (activeRemotes.length) {
			...
			Logger.appendLine(
				`No GitHub remotes found for folder ${this.repository.rootUri.fsPath}`,
			);
		}
	...
	private async doUpdateRepositories(silent: boolean): Promise<boolean> {
		if (this._git.state === "uninitialized") {
			Logger.appendLine(
				"Cannot updates repositories as git is uninitialized",
			);
		...
		if (
			this.credentialStore.isAnyAuthenticated() &&
			activeRemotes.length === 0
		) {
			const areAllNeverGitHub = (
				await this.computeAllUnknownRemotes()
			).every((remote) =>
				GitHubManager.isNeverGitHub(
					vscode.Uri.parse(remote.normalizedHost).authority,
				),
			);
		...
		for (const remote of authenticatedRemotes) {
			const repository = await this.createGitHubRepository(
				remote,
		...
		return Promise.all(resolveRemotePromises).then(
			async (remoteResults: boolean[]) => {
				...
				if (missingSaml.length > 0) {
					const result =
						await this._credentialStore.showSamlMessageAndAuth(
							missingSaml.map((repo) => repo.remote.owner),
						);
					// Make a test call to see if the user has SAML enabled.
					const samlTest = result.canceled
						? []
						: await Promise.all(
								missingSaml.map((repo) => repo.resolveRemote()),
							);

					const stillMissing = result.canceled
						? missingSaml
						: samlTest
								.map((result, index) =>
									!result ? missingSaml[index] : undefined,
								)
								.filter(
									(repo): repo is GitHubRepository => !!repo,
								);
					// Make a test call to see if the user has SAML enabled.
					if (stillMissing.length > 0) {
						if (stillMissing.length === repositories.length) {
							await vscode.window.showErrorMessage(
								vscode.l10n.t(
									"SAML access was not provided. GitHub Pull Requests will not work.",
								),
								{ modal: true },
							);
							this.dispose();
				...
				const repositoriesAdded =
					oldRepositories.length !== this._githubRepositories.length
						? this.gitHubRepositories.filter(
								(repo) =>
									!oldRepositories.some((oldRepo) =>
										oldRepo.remote.equals(repo.remote),
									),
							)
						: [];

				if (repositoriesAdded.length > 0) {
					this._onDidChangeGithubRepositories.fire(
						this._githubRepositories,
					);
				}

				if (
					this._githubRepositories.length &&
	...
	private async checkIfMissingUpstream(): Promise<boolean> {
		try {
			...
			if (
				metadata.fork &&
				metadata.parent &&
				configuration.get<"add" | "never">(UPSTREAM_REMOTE, "add") ===
					"add"
			) {
				...
				const missingParentRemote = !this._githubRepositories.some(
					(repo) =>
						...
						compareIgnoreCase(
							...
							parentUrl.repositoryName,
						) === 0,
				);

				if (missingParentRemote) {
					const upstreamAvailable =
						!this.repository.state.remotes.some(
							(remote) => remote.name === "upstream",
						);

					const remoteName = upstreamAvailable
						? "upstream"
						: metadata.parent.owner?.login;

					if (remoteName) {
						// check the remotes to see what protocol is being used
						const isSSH =
							this.gitHubRepositories[0].remote.gitProtocol
								.type === ProtocolType.SSH;

						if (isSSH) {
							await this.repository.addRemote(
								remoteName,
								metadata.parent.ssh_url,
							);
						} else {
							await this.repository.addRemote(
								remoteName,
						...
						if (this._addedUpstreamCount > 1) {
							...
							vscode.window
								.showInformationMessage(
									vscode.l10n.t(
										"An `upstream` remote has been added for this repository. You can disable this feature by setting `githubPullRequests.upstreamRemote` to `never`.",
									),
									neverOption,
								)
								.then((choice) => {
	...
	private async getCachedFromGlobalState<T>(
		userKind:
			| "assignableUsers"
			| "teamReviewers"
			| "mentionableUsers"
		...
		const usersCacheLocation = vscode.Uri.joinPath(
		...
		try {
			usersCacheExists =
				await vscode.workspace.fs.stat(usersCacheLocation);
		} catch (e) {
			// file doesn't exit
		}
		if (!usersCacheExists) {
		...
		const hasAllRepos = (
			await Promise.all(
				this._githubRepositories.map(async (repo) => {
					...
					const repoSpecificFile = vscode.Uri.joinPath(
						usersCacheLocation,
						key,
					);
					...
					try {
						...
						if (
							...
							Logger.error(
								`Error parsing ${userKind} cache for ${repo.remote.remoteName}.`,
							);
						}
						// file doesn't exist
					}
		...
		if (hasAllRepos) {
			Logger.appendLine(
				`Using globalState ${userKind} for ${Object.keys(cache).length}.`,
			);
	...
	private async saveInGlobalState<T>(
		userKind:
			| "assignableUsers"
			| "teamReviewers"
			| "mentionableUsers"
		...
		const cacheLocation = vscode.Uri.joinPath(
			this.context.globalStorageUri,
			userKind,
		);
		await Promise.all(
			this._githubRepositories.map(async (repo) => {
				...
				await vscode.workspace.fs.writeFile(
					...
					new TextEncoder().encode(
						JSON.stringify(cache[repo.remote.remoteName]),
					),
				);
			}),
		);
	...
	private createFetchMentionableUsersPromise(): Promise<{
		[key: string]: IAccount[];
	}> {
	...
	async getMentionableUsers(
		...
		const globalStateMentionableUsers =
			await this.getCachedFromGlobalState<IAccount>("mentionableUsers");

		if (!this._fetchMentionableUsersPromise) {
			this._fetchMentionableUsersPromise =
				this.createFetchMentionableUsersPromise();

			return (
				globalStateMentionableUsers ??
				this._fetchMentionableUsersPromise
			);
		}
	...
	async getAssignableUsers(
		...
		const globalStateAssignableUsers =
			await this.getCachedFromGlobalState<IAccount>("assignableUsers");
		...
		if (!this._fetchAssignableUsersPromise) {
			...
			return (
				globalStateAssignableUsers ?? this._fetchAssignableUsersPromise
			);
		}
	...
	async getTeamReviewers(
		refreshKind: TeamReviewerRefreshKind,
	): Promise<{ [key: string]: ITeam[] }> {
		...
		const globalStateTeamReviewers =
			refreshKind === TeamReviewerRefreshKind.Force
				? undefined
				: await this.getCachedFromGlobalState<ITeam>("teamReviewers");
				...
				async (resolve) => {
					...
					for (const githubRepository of this._githubRepositories) {
						...
						cache[githubRepository.remote.remoteName] =
							allTeamsForOrg
								.filter((team) =>
									team.repositoryNames.includes(
										githubRepository.remote.repositoryName,
									),
								)
								.sort(teamComparator);
					}
	...
	private createFetchOrgProjectsPromise(): Promise<{
		[key: string]: IProject[];
	}> {
	...
	async getOrgProjects(
		clearCache?: boolean,
	): Promise<{ [key: string]: IProject[] }> {
		if (clearCache) {
			return this.createFetchOrgProjectsPromise();
		}

		const globalStateProjects =
			await this.getCachedFromGlobalState<IProject>("orgProjects");
	...
	async getAllProjects(
		...
		clearOrgCache?: boolean,
	): Promise<IProject[]> {
		const isInOrganization = !!(await githubRepository.getMetadata())
			.organization;

		const [repoProjects, orgProjects] = await Promise.all([
			githubRepository.getProjects(),
			isInOrganization ? this.getOrgProjects(clearOrgCache) : undefined,
		]);

		return [
			...(repoProjects ?? []),
			...(orgProjects
				? orgProjects[githubRepository.remote.remoteName]
				: []),
		];
	}
	...
	): Promise<{ participants: IAccount[]; viewer: IAccount }> {
		return {
			participants:
				await githubRepository.getPullRequestParticipants(
					pullRequestNumber,
				),
	...
	async getGitHubRemotes(): Promise<GitHubRemote[]> {
		...
		const githubRemotes = remotes.map((remote, index) =>
			GitHubRemote.remoteAsGitHub(remote, serverTypes[index]),
		);
	...
	async getLocalPullRequests(): Promise<PullRequestModel[]> {
		...
		const localBranches = (
			await this.repository.getRefs({ pattern: "refs/heads/" })
		)
		...
		for (const chunk of chunkedLocalBranches) {
			models.push(
				...(await Promise.all(
					chunk.map(async (localBranchName) => {
						const matchingPRMetadata =
							await PullRequestGitHelper.getMatchingPullRequestMetadataForBranch(
								this.repository,
								localBranchName,
							);
						...
						if (matchingPRMetadata) {
							...
							const githubRepo = githubRepositories.find(
								(repo) =>
									repo.remote.owner.toLocaleLowerCase() ===
							...
							if (githubRepo) {
								const pullRequest:
									| PullRequestModel
									| undefined =
									await githubRepo.getPullRequest(prNumber);
	...
	async getLabels(
		...
		const repo = issue
			...
			: this._githubRepositories.find(
					(r) =>
						r.remote.owner === repoInfo?.owner &&
						r.remote.repositoryName === repoInfo?.repo,
				);
		...
		do {
			const result = await octokit.call(
				octokit.api.issues.listLabelsForRepo,
				{
					owner: remote.owner,
					repo: remote.repositoryName,
					per_page: 100,
	...
	async deleteLocalPullRequest(
		...
		try {
			remoteName = await this.repository.getConfig(
				`branch.${pullRequest.localBranchName}.remote`,
			);
		} catch (e) {}
		...
		const isPRRemote =
			await PullRequestGitHelper.isRemoteCreatedForPullRequest(
				this.repository,
				remoteName,
			);
		...
		if (isPRRemote) {
			...
			const hasOtherAssociatedBranches = configs.some(
				({ key, value }) =>
					/^branch.*\.remote$/.test(key) && value === remoteName,
			);
	...
	private async fetchPagedData<T>(
		...
		const githubRepositoriesWithGitRemotes =
			pagedDataType === PagedDataType.PullRequest
				? this._githubRepositories.filter((repo) =>
						this.repository.state.remotes.find(
							(r) => r.name === repo.remote.remoteName,
						),
					)
				: this._githubRepositories;
		...
		const getTotalFetchedPages = () =>
			this.totalFetchedPages.get(queryId) || 0;

		const setTotalFetchedPages = (numPages: number) =>
			this.totalFetchedPages.set(queryId, numPages);
		...
		const githubRepositories = this._githubRepositories.filter((repo) => {
			const info = this._repositoryPageInformation.get(
				repo.remote.url.toString() + queryId,
			);
			// If we are in case 1 or 3, don't filter out repos that are out of pages, as we will be querying from the start.
			return (
				info &&
				(options.fetchNextPage === false || info.hasMorePages !== false)
			);
		});
		...
		for (let i = 0; i < githubRepositories.length; i++) {
			...
			const fetchPage = async (
				...
				| { items: any[]; hasMorePages: boolean; totalCount?: number }
				| undefined
			> => {
				// Resolve variables in the query with each repo
				const resolvedQuery = query
					? await variableSubstitution(query, undefined, {
							base: await githubRepository.getDefaultBranch(),
				...
				switch (pagedDataType) {
					case PagedDataType.PullRequest: {
						if (type === PRType.All) {
							return githubRepository.getAllPullRequests(
								pageNumber,
							);
						} else {
							return githubRepository.getPullRequestsForCategory(
								resolvedQuery || "",
								pageNumber,
							);
						}
			...
			if (options.fetchNextPage) {
				...
				const pages = await Promise.all(
					Array.from({ length: pageInformation.pullRequestPage }).map(
						(_, j) => fetchPage(j + 1),
					),
				);
			...
			if (
				...
				(options.fetchNextPage ||
					(options.fetchNextPage === false &&
						!options.fetchOnePagePerRepo &&
						pagesFetched >= getTotalFetchedPages()))
			) {
				...
				return {
					...
					hasUnsearchedRepositories:
						i < githubRepositories.length - 1,
	...
	async getPullRequests(
		...
		return this.fetchPagedData<PullRequestModel>(
			options,
			queryId,
			PagedDataType.PullRequest,
			type,
			query,
		);
	}
	...
	async createMilestone(
		...
		try {
			const { data } = await repository.octokit.call(
				repository.octokit.api.issues.createMilestone,
				{
	...
	private async getRepoForIssue(
		...
		return this.createGitHubRepository(
			remote,
	...
	async getIssues(
		...
		try {
			...
			Logger.error(
				`Error fetching issues with query ${query}: ${e instanceof Error ? e.message : e}`,
				this.id,
			);
	...
	async getIssueTemplates(): Promise<vscode.Uri[]> {
		...
		return vscode.workspace.findFiles(
			new vscode.RelativePattern(this._repository.rootUri, pattern),
			null,
		);
	}

	async getPullRequestTemplateBody(
		owner: string,
		...
		try {
			...
			Logger.error(
				`Error fetching pull request template for ${owner}: ${e instanceof Error ? e.message : e}`,
				this.id,
			);
		}
	}

	private async getPullRequestTemplateWithCache(
		owner: string,
	): Promise<string | undefined> {
		const cacheLocation = `${CACHED_TEMPLATE_BODY}+${this.repository.rootUri.toString()}`;

		const findTemplate = this.getPullRequestTemplate(owner).then(
			(template) => {
				//update cache
				if (template) {
		...
		const hasCachedTemplate = this.context.workspaceState
			.keys()
			.includes(cacheLocation);

		const cachedTemplate = this.context.workspaceState.get<string | null>(
			cacheLocation,
		);
	...
	private async getOwnerPullRequestTemplate(
		owner: string,
	): Promise<string | undefined> {
		const githubRepository = await this.createGitHubRepositoryFromOwnerName(
			owner,
			".github",
		);
	...
	private async getPullRequestTemplate(
		owner: string,
	): Promise<string | undefined> {
		const repository = this.gitHubRepositories.find(
	...
	async getPullRequestDefaults(
		...
		const remotesSettingSetValue = vscode.workspace
			.getConfiguration(PR_SETTINGS_NAMESPACE)
			.get<string[]>(REMOTES);

		const settingsEqual =
			!remotesSettingSetValue ||
			remotesSettingDefault?.every(
				(value, index) => remotesSettingSetValue[index] === value,
			);

		const parent =
			meta.fork && meta.parent && settingsEqual
				? meta.parent
				: await (
						this.findRepo(byRemoteName("upstream")) || origin
					).getMetadata();
	...
	async getPullRequestDefaultRepo(): Promise<GitHubRepository> {
		...
		return (
			this.findRepo(
				(repo) =>
					repo.remote.owner === defaults.owner &&
					repo.remote.repositoryName === defaults.repo,
			) || this._githubRepositories[0]
		);
	}
	...
	async getHeadCommitMessage(): Promise<string> {
		...
		if (repository.state.HEAD && repository.state.HEAD.commit) {
			const { message } = await repository.getCommit(
	...
	async getOrigin(branch?: Branch): Promise<GitHubRepository> {
		...
		if (upstreamRef) {
			...
			if (!upstream) {
				const remote = (await this.getAllGitHubRemotes()).find(
					(r) => r.remoteName === upstreamRef.remote,
				);

				if (remote) {
					return this.createAndAddGitHubRepository(
						remote,
						this._credentialStore,
					);
				}
				...
				Logger.error(
					`The remote '${upstreamRef.remote}' is not a GitHub repository.`,
				);
	...
	async createPullRequest(
		...
		const repo = this._githubRepositories.find(
			(r) =>
				r.remote.owner === params.owner &&
				r.remote.repositoryName === params.repo,
		);

		if (!repo) {
			throw new Error(
				`No matching repository ${params.repo} found for ${params.owner}`,
			);
		}
		...
		try {
			...
			*/
			this.telemetry.sendTelemetryEvent("pr.create.success", {
			...
			if (e.message.indexOf("No commits between ") > -1) {
				...
				if (this._repository.state.HEAD?.ahead) {
					...
					const pushCommits = vscode.l10n.t({
						message: "Push Commits",
						comment: "Pushes the local commits to the remote.",
					});

					const shouldPush =
						await vscode.window.showInformationMessage(
							vscode.l10n.t(
								"There are no commits between '{0}' and '{1}'.\n\nDo you want to push your local commits and create the pull request?",
								params.base,
								params.head,
							),
							{ modal: true },
				...
				if (
					this._repository.state.workingTreeChanges.length ||
					this._repository.state.indexChanges.length
				) {
					...
					if (shouldCommit === commitChanges) {
						await this._repository.add(
							this._repository.state.indexChanges.map(
	...
	async createIssue(
		...
		try {
			...
			// Create PR
			const { data } = await repo.octokit.call(
				repo.octokit.api.issues.create,
				params,
			);
	...
	async assignIssue(issue: IssueModel, login: string): Promise<void> {
		try {
			const repo = this._githubRepositories.find(
				(r) =>
					r.remote.owner === issue.remote.owner &&
					r.remote.repositoryName === issue.remote.repositoryName,
			);
	...
	async mergePullRequest(
		...
		Logger.debug(
			`Merging PR: ${pullRequest.number} method: ${method} for user: "${email}" - enter`,
			this.id,
		);
		...
		const activePRSHA =
			...
			this.activePullRequest.head &&
			this.activePullRequest.head.sha;

		const workingDirectorySHA =
			this.repository.state.HEAD && this.repository.state.HEAD.commit;

		const mergingPRSHA = pullRequest.head && pullRequest.head.sha;

		const workingDirectoryIsDirty =
		...
		if (activePRSHA === mergingPRSHA) {
			...
			if (workingDirectorySHA !== mergingPRSHA) {
				...
				const pluralMessage = vscode.l10n.t(
					"You have {0} unpushed commits on this PR branch.\n\nWould you like to proceed anyway?",
					ahead ?? "unknown",
				);

				const singularMessage = vscode.l10n.t(
					"You have 1 unpushed commit on this PR branch.\n\nWould you like to proceed anyway?",
				);
		...
		const input: MergePullRequestInput = {
			...
			mergeMethod: (method?.toUpperCase() ??
				vscode.workspace
					...
					?.toUpperCase()) as GraphQLMergeMethod,
		};
		...
		return mutate<MergePullRequestResponse>({
			...
			.then((result) => {
				Logger.debug(
					`Merging PR: ${pullRequest.number}} - done`,
					this.id,
				);
				...
				*/
				...
				return {
					merged: true,
					message: "",
					timeline: parseGraphQLTimelineEvents(
						result.data?.mergePullRequest.pullRequest.timelineItems
							.nodes ?? [],
			...
			.catch((e) => {
				...
				*/
				...
				if (
					...
					graphQLErrors.find(
						(error) =>
							error.type === GraphQLErrorType.Unprocessable &&
					...
					return {
						...
						message: vscode.l10n.t(
							"Head branch was modified. Pull, review, then try again.",
						),
					};
				} else {
					throw e;
	...
	private async getBranchDeletionItems() {
		...
		const branchInfos: Map<
			string,
			{ remote?: string; metadata?: PullRequestMetadata }
		> = new Map();
		...
		branchInfos.forEach((value, key) => {
			if (value.metadata) {
				const activePRUrl =
					this.activePullRequest &&
					this.activePullRequest.base.repositoryCloneUrl;

				const matchesActiveBranch = activePRUrl
					? activePRUrl.owner === value.metadata.owner &&
						activePRUrl.repositoryName ===
							value.metadata.repositoryName &&
						this.activePullRequest &&
						this.activePullRequest.number ===
		...
		const results = await Promise.all(
			actions.map(async (action) => {
				...
				const githubRepo = this._githubRepositories.find(
					(repo) =>
						repo.remote.owner.toLowerCase() ===
							metadata!.owner.toLowerCase() &&
						repo.remote.repositoryName.toLowerCase() ===
							metadata!.repositoryName.toLowerCase(),
				);
		...
		results.forEach((result) => {
			if (result.legacy) {
				...
				result.description = vscode.l10n.t(
					"{0} is still Open",
					result.description!,
				);
			}
	...
	public gitRelativeRootPath(path: string) {
		...
		return nodePath
			.relative(this._repository.rootUri.path, path)
			.replace(/\\/g, "/");
	}

	public async cleanupAfterPullRequest(
		branchName: string,
		pullRequest: PullRequestModel,
	) {
		const defaults = await this.getPullRequestDefaults();
	...
	private async getRemoteDeletionItems(nonExistantBranches: Set<string>) {
		...
		const remoteInfos =
			await this.getDeleatableRemotes(nonExistantBranches);

		const remoteItems: (vscode.QuickPickItem & { remote: string })[] = [];

		remoteInfos.forEach((value, key) => {
			if (value.branches.size === 0) {
				let description = value.createdForPullRequest
					? ""
					: vscode.l10n.t(
							"Not created by GitHub Pull Request extension",
						);
	...
	private async deleteBranches(
		picks: readonly vscode.QuickPickItem[],
		nonExistantBranches: Set<string>,
		progress: vscode.Progress<{ message?: string; increment?: number }>,
		totalBranches: number,
		deletedBranches: number,
		needsRetry?: vscode.QuickPickItem[],
	) {
		...
		await batchPromiseAll(picks, 5, async (pick) => {
			try {
				...
				if (
					...
					typeof e.stderr === "string" &&
					e.stderr.includes("unable to access") &&
					needsRetry
				) {
		...
		return new Promise<void>(async (resolve) => {
			...
			quickPick.selectedItems = results.filter((result) => {
				...
				return (
					result.picked &&
					!(
						result.label === defaults.base &&
						result.metadata.owner === defaults.owner &&
						result.metadata.repositoryName === defaults.repo
					)
				);
			});
			...
			quickPick.onDidAccept(async () => {
				...
				if (firstStep) {
						...
						await vscode.window.withProgress(
							{
								...
								title: vscode.l10n.t("Cleaning up"),
							},
							async (progress) => {
								try {
									await this.deleteBranches(
					...
					const remoteItems =
						await this.getRemoteDeletionItems(nonExistantBranches);
	...
	async revert(
		...
		const repo = this._githubRepositories.find(
			(r) =>
				r.remote.owner === pullRequest.remote.owner &&
				r.remote.repositoryName === pullRequest.remote.repositoryName,
		);

		if (!repo) {
			throw new Error(
				`No matching repository ${pullRequest.remote.repositoryName} found for ${pullRequest.remote.owner}`,
			);
		}

		const pullRequestModel: PullRequestModel | undefined =
			await repo.revertPullRequest(
				pullRequest.graphNodeId,
				title,
				body,
				draft,
			);
	...
	async getPullRequestRepositoryDefaultBranch(
		issue: IssueModel,
	): Promise<string> {
	...
	async getPullRequestRepositoryAccessAndMergeMethods(
		...
		const mergeOptions =
			await pullRequest.githubRepository.getRepoAccessAndMergeMethods();
	...
	async mergeQueueMethodForBranch(
		...
		return await this.gitHubRepositories
			.find(
				(repository) =>
					repository.remote.owner === owner &&
					repository.remote.repositoryName === repoName,
			)
			?.mergeQueueMethodForBranch(branch);
	}

	async fulfillPullRequestMissingInfo(
		pullRequest: PullRequestModel,
	): Promise<void> {
		try {
			...
			vscode.window.showErrorMessage(
				vscode.l10n.t(
					"Fetching Pull Request merge base failed: {0}",
					formatError(e),
				),
			);
		}
	...
	private async resolveItem(
		...
		if (!githubRepo) {
			Logger.appendLine(
				`GitHubRepository not found: ${owner}/${repositoryName}`,
				this.id,
			);
			// try to create the repository
			githubRepo = await this.createGitHubRepositoryFromOwnerName(
				owner,
				repositoryName,
			);
		}
	...
	async resolveIssueOrPullRequest(
		owner: string,
		repositoryName: string,
		issueOrPullRequestNumber: number,
	): Promise<PullRequestModel | IssueModel | undefined> {
		let issueOrPullRequest: IssueModel | PullRequestModel | undefined =
		...
		if (!issueOrPullRequest) {
			issueOrPullRequest = await this.resolvePullRequest(
				owner,
				repositoryName,
				issueOrPullRequestNumber,
			);
	...
	async resolveUser(
		...
		try {
			...
			if (
				!(e.message as string | undefined)?.startsWith(
	...
	async getMatchingPullRequestMetadataFromGitHub(
		branch: Branch,
		remoteName?: string,
		remoteUrl?: string,
		upstreamBranchName?: string,
	): Promise<(PullRequestMetadata & { model: PullRequestModel }) | null> {
		try {
			if (remoteName) {
				return this.getMatchingPullRequestMetadataFromGitHubWithRemoteName(
					remoteName,
					upstreamBranchName,
				);
			}
			...
			Logger.error(
				`Unable to get matching pull request metadata from GitHub: ${e}`,
				this.id,
			);
	...
	async getMatchingPullRequestMetadataFromGitHubWithUrl(
		branch: Branch,
		remoteUrl?: string,
		upstreamBranchName?: string,
	): Promise<(PullRequestMetadata & { model: PullRequestModel }) | null> {
		...
		if (!headGitHubRepo && this.gitHubRepositories.length > 0) {
			...
			if (remote) {
				headGitHubRepo = await this.createGitHubRepository(
					remote,
					this.credentialStore,
					true,
					true,
		...
		if (
			...
			const newRemote = await PullRequestGitHelper.createRemote(
				this.repository,
				headGitHubRepo?.remote,
				protocol,
			);
	...
	async getMatchingPullRequestMetadataFromGitHubWithRemoteName(
		remoteName?: string,
		upstreamBranchName?: string,
	): Promise<(PullRequestMetadata & { model: PullRequestModel }) | null> {
		if (!remoteName) {
		...
		return this.doGetMatchingPullRequestMetadataFromGitHub(
			headGitHubRepo,
			upstreamBranchName,
		);
	}

	private async doGetMatchingPullRequestMetadataFromGitHub(
		headGitHubRepo?: GitHubRepository,
		upstreamBranchName?: string,
	): Promise<(PullRequestMetadata & { model: PullRequestModel }) | null> {
		...
		const parentRepos = this.gitHubRepositories.filter((repo) => {
			if (headRepoMetadata.fork) {
				return (
					repo.remote.owner ===
						headRepoMetadata.parent?.owner?.login &&
					repo.remote.repositoryName === headRepoMetadata.parent.name
				);
			} else {
				return (
					repo.remote.owner === headRepoMetadata.owner?.login &&
					repo.remote.repositoryName === headRepoMetadata.name
				);
			}
		});
	...
	async checkoutExistingPullRequestBranch(
		pullRequest: PullRequestModel,
		progress: vscode.Progress<{ message?: string; increment?: number }>,
	): Promise<boolean> {
		return await PullRequestGitHelper.checkoutExistingPullRequestBranch(
			this.repository,
			pullRequest,
			progress,
		);
	...
	async getBranchNameForPullRequest(pullRequest: PullRequestModel) {
		return await PullRequestGitHelper.getBranchNRemoteForPullRequest(
			this.repository,
			pullRequest,
		);
	}
	...
	async fetchAndCheckout(
		...
		await PullRequestGitHelper.fetchAndCheckout(
			this.repository,
			this._allGitHubRemotes,
			pullRequest,
			progress,
	...
	async tryMergeBaseIntoHead(
		...
		if (!pullRequest.isActive || isBrowser) {
			const conflictModel = await vscode.window.withProgress(
				{
					location: vscode.ProgressLocation.Notification,
					title: vscode.l10n.t("Finding conflicts..."),
				},
				() => createConflictResolutionModel(pullRequest),
			);

			if (conflictModel === undefined) {
				await vscode.window.showErrorMessage(
					vscode.l10n.t(
						"Unable to resolved conflicts for this pull request. There are too many file changes.",
					),
					{
						modal: true,
						detail: isBrowser
							? undefined
							: vscode.l10n.t(
									"Please check out the pull request to resolve conflicts.",
								),
					},
				);
			...
			if (
				...
				const githubRepos = await Promise.all([
					this.createGitHubRepositoryFromOwnerName(
						pullRequest.head!.owner,
						pullRequest.head!.repositoryCloneUrl.repositoryName,
					),
					this.createGitHubRepositoryFromOwnerName(
						pullRequest.base.owner,
						pullRequest.base.repositoryCloneUrl.repositoryName,
					),
				]);

				const coordinator = new ConflictResolutionCoordinator(
					this.telemetry,
					conflictModel,
					githubRepos,
				);
		...
		if (pullRequest.item.mergeable === PullRequestMergeability.Conflict) {
			const wizard = await ConflictModel.begin(
				this.repository,
				pullRequest.base.ref,
				this.repository.state.HEAD!.name!,
				push,
	...
	async fetchById(
		...
		if (pullRequest) {
			...
			vscode.window.showErrorMessage(
				vscode.l10n.t(
					"Pull request number {0} does not exist in {1}",
					id,
					`${githubRepo.remote.owner}/${githubRepo.remote.repositoryName}`,
				),
				{ modal: true },
			);
		}
		...
		try {
			...
			if (
				vscode.workspace
					...
					.get<boolean>(PULL_BEFORE_CHECKOUT, false) &&
				branchObj.upstream
			) {
				await this.repository.fetch({
					remote: branchObj.upstream.remote,
					ref: `${branchObj.upstream.name}:${branchObj.name}`,
				});
			}
			...
			for (const tabGroup of vscode.window.tabGroups.all) {
				for (const tab of tabGroup.tabs) {
					...
					if (
						...
						(typeof uri === "string" &&
							uri.endsWith(PULL_REQUEST_OVERVIEW_VIEW_TYPE))
					) {
	...
	private async pullBranchConfiguration(): Promise<
		"never" | "prompt" | "always"
	> {
		const neverShowPullNotification = this.context.globalState.get<boolean>(
			NEVER_SHOW_PULL_NOTIFICATION,
			false,
		);
	...
	private async promptPullBrach(
		pr: PullRequestModel,
		branch: Branch,
		autoStashSetting?: boolean,
	) {
	...
	public async checkBranchUpToDate(
		...
		if (branch) {
			...
			const remoteBranch = branch.upstream
				...
				try {
					...
					if (e.stderr) {
						if (
							...
							vscode.window.showErrorMessage(
								vscode.l10n.t(
									"An error occurred when fetching the repository: {0}",
									e.stderr,
								),
							);
						}
					...
					switch (pullBranchConfiguration) {
						case "always": {
							const autoStash = vscode.workspace
								.getConfiguration(GIT)
								.get<boolean>(AUTO_STASH, false);

							if (autoStash) {
								return this.promptPullBrach(
									pr,
									branch,
									autoStash,
								);
	...
	public findExistingGitHubRepository(remote: {
		...
		return this._githubRepositories.find(
			(r) =>
				...
				r.remote.repositoryName.toLowerCase() ===
					remote.repositoryName.toLowerCase() &&
				(!remote.remoteName ||
					r.remote.remoteName === remote.remoteName),
		);
	}

	private async createAndAddGitHubRepository(
		remote: Remote,
		...
		const repo = new GitHubRepository(
			...
			GitHubRemote.remoteAsGitHub(
				remote,
				await this._githubManager.isGitHub(
					remote.gitProtocol.normalizeUri()!,
				),
			),
	...
	async createGitHubRepository(
		...
		credentialStore: CredentialStore,
		silent?: boolean,
		ignoreRemoteName: boolean = false,
	): Promise<GitHubRepository> {
		...
		return this._createGitHubRepositoryBulkhead.execute(async () => {
			return (
				this.findExistingGitHubRepository({
					owner: remote.owner,
					repositoryName: remote.repositoryName,
					remoteName: ignoreRemoteName
						? undefined
						: remote.remoteName,
				}) ??
				(await this.createAndAddGitHubRepository(
					remote,
					credentialStore,
					silent,
				))
	...
	async createGitHubRepositoryFromOwnerName(
		owner: string,
		repositoryName: string,
	): Promise<GitHubRepository> {
		const existing = this.findExistingGitHubRepository({
			owner,
			repositoryName,
		});
		...
		const gitRemote = gitRemotes.find(
			(r) => r.owner === owner && r.repositoryName === repositoryName,
		);

		const uri =
			gitRemote?.url ?? `https://github.com/${owner}/${repositoryName}`;

		return this.createAndAddGitHubRepository(
			new Remote(
				gitRemote?.remoteName ?? repositoryName,
				uri,
				new Protocol(uri),
			),
			this._credentialStore,
	...
	async findUpstreamForItem(item: {
		...
		for (const githubRepo of this.gitHubRepositories) {
			...
			if (
				...
				const foundforkPermission =
					await githubRepo.getViewerPermission();
		...
		if (upstream && !existingForkRemote) {
			const permission =
				await item.githubRepository.getViewerPermission();
	...
	async forkWithProgress(
		...
		progress.report({
			message: vscode.l10n.t("Forking {0}...", repoString),
		});
		...
		if (!result) {
			vscode.window.showErrorMessage(
				vscode.l10n.t(
					"Unable to create a fork of {0}. Check that your GitHub credentials are correct.",
					repoString,
				),
			);
	...
	async doFork(
		...
		return vscode.window.withProgress<string | undefined>(
			{
				location: vscode.ProgressLocation.Notification,
				title: vscode.l10n.t("Creating Fork"),
			},
			async (progress) => {
				try {
					return this.forkWithProgress(
						progress,
						githubRepository,
						repoString,
						matchingRepo,
	...
	async tryOfferToFork(
		...
		const response = await vscode.window.showInformationMessage(
			vscode.l10n.t(
				"You don't have permission to push to {0}. Do you want to fork {0}? This will modify your git remotes to set `origin` to the fork, and `upstream` to {0}.",
				repoString,
			),
			{ modal: true },
			fork,
...
export const byRemoteName =
	(name: string): Predicate<GitHubRepository> =>
	({ remote: { remoteName } }) =>
		remoteName === name;

export const titleAndBodyFrom = async (
