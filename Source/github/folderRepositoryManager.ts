...
export class FolderRepositoryManager implements vscode.Disposable {
	...
	private _onDidChangeActivePullRequest = new vscode.EventEmitter<{
	...
	private _onDidLoadRepositories =
	...
	private _onDidChangeGithubRepositories = new vscode.EventEmitter<
		GitHubRepository[]
	>();
			...
			async (remoteResults: boolean[]) => {
				...
				if (missingSaml.length > 0) {
					...
					if (result.canceled) {
						...
						const samlTest = await Promise.all(
