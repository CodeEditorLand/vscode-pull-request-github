...
class MergeOutputProvider implements vscode.FileSystemProvider {
	...
	clear(): void {
		...
		for (const file of this._mergedFiles.keys()) {
			fileEvents.push({
				uri: vscode.Uri.from({
					scheme: this._conflictResolutionModel.mergeScheme,
					path: file,
				}),
...
export class ConflictResolutionCoordinator {
	...
	constructor(
		...
		this._mergeOutputProvider = new MergeOutputProvider(
			this._conflictResolutionModel,
		);
	...
	private async openConflict(conflict: Conflict) {
		...
		const prHead: MergeEditorInputData = {
			uri: prHeadUri,
			title: vscode.l10n.t("Pull Request Head"),
		};
		const base: MergeEditorInputData = {
			uri: baseUri,
			title: vscode.l10n.t(
				"{0} Branch",
				this._conflictResolutionModel.prBaseBranchName,
			),
		};

		const mergeBaseUri: vscode.Uri =
			this._conflictResolutionModel.mergeBaseUri(conflict);
		const mergeOutput =
			this._conflictResolutionModel.mergeOutputUri(conflict);
		const options = {
			base: mergeBaseUri,
			input1: prHead,
			input2: base,
	...
	private register(): void {
		this._disposables.push(
			vscode.workspace.registerFileSystemProvider(
				Schemes.GithubPr,
				new GitHubContentProvider(this._githubRepositories),
				{ isReadonly: true },
			),
		);
		this._disposables.push(
			...
			vscode.commands.registerCommand(
				...
				async () => {
					...
					const result = await vscode.window.showWarningMessage(
						vscode.l10n.t(
							"Are you sure you want to exit conflict resolution mode? All changes will be lost.",
						),
						{ modal: true },
						exit,
					);
	...
	private async acceptMerge(uri: vscode.Uri | unknown): Promise<void> {
		...
		if (
			!activeTab ||
			!(activeTab.input instanceof vscode.TabInputTextMerge)
		) {
			return;
		}
	...
	async exitConflictResolutionMode(
		...
		*/
		this._telemetry.sendTelemetryEvent("pr.conflictResolution.exit", {
			allConflictsResolved: allConflictsResolved.toString(),
		});
		...
		for (const group of vscode.window.tabGroups.all) {
			for (const tab of group.tabs) {
				if (
					tab.input instanceof vscode.TabInputTextMerge &&
					tab.input.result.scheme ===
						this._conflictResolutionModel.mergeScheme
				) {
