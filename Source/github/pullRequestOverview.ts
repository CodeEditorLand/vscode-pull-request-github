...
export class PullRequestOverviewPanel extends IssueOverviewPanel<PullRequestModel> {
	...
	public static override async createOrShow(
		...
		if (PullRequestOverviewPanel.currentPanel) {
			PullRequestOverviewPanel.currentPanel._panel.reveal(
				activeColumn,
				preserveFocus,
			);
		} else {
		...
		await PullRequestOverviewPanel.currentPanel!.update(
			folderRepositoryManager,
			issue,
		);
	}

	protected override set _currentPanel(
		panel: PullRequestOverviewPanel | undefined,
	) {
	...
	public static scrollToReview(): void {
		if (this.currentPanel) {
			this.currentPanel._postMessage({
				command: "pr.scrollToPendingReview",
			});
		}
	}
	...
	protected constructor(
		...
		this._register(
			vscode.commands.registerCommand("review.approveDescription", (e) =>
	...
	registerPrListeners() {
		...
		if (this._item) {
			this._prListeners.push(
				this._item.onDidChangeComments(() => {
					if (!this._isUpdating) {
						this.refreshPanel();
					}
				}),
	...
	private setVisibilityContext() {
		return commands.setContext(
			contexts.PULL_REQUEST_DESCRIPTION_VISIBLE,
			this._panel.visible,
		);
	}
	...
	private isUpdateBranchWithGitHubEnabled(): boolean {
		return (
			...
			vscode.workspace
				...
				.get("experimentalUpdateBranchWithGitHub", false)
		);
	}
	...
	private async updatePullRequest(
		...
		return Promise.all([
			...
			this._folderRepositoryManager.getOrgTeamsCount(
				pullRequestModel.githubRepository,
			),
			this._folderRepositoryManager.mergeQueueMethodForBranch(
				pullRequestModel.base.ref,
				pullRequestModel.remote.owner,
				pullRequestModel.remote.repositoryName,
			),
			this._folderRepositoryManager.isHeadUpToDateWithBase(
				pullRequestModel,
			),
			pullRequestModel.getMergeability(),
			this._folderRepositoryManager.credentialStore.getIsEmu(
				pullRequestModel.remote.authProviderId,
			),
		])
			.then((result) => {
				const [
				...
				const isUpdateBranchWithGitHubEnabled: boolean =
				...
				const context: Partial<PullRequest> = {
					...
					canUpdateBranch:
						pullRequest.item.viewerCanUpdate &&
						!isBranchUpToDateWithBase &&
				...
				if (pullRequest.isResolved()) {
					this._folderRepositoryManager.checkBranchUpToDate(
						pullRequest,
						true,
					);
				}
	...
	public override async update(
		...
		if (
			!this._item ||
			this._item.number !== pullRequestModel.number ||
		...
		switch (message.command) {
			...
			case "pr.checkMergeability":
				return this._replyMessage(
					message,
					await this._item.getMergeability(),
				);
	...
	private async changeReviewers(
		...
		try {
			quickPick = await reviewersQuickPick(
				...
				this._item.remote.remoteName,
				this._item.base.isInOrganization,
				this._teamsCount,
				this._item.author,
				this._existingReviewers,
			...
			if (allReviewers) {
				...
				this._existingReviewers.forEach((existing) => {
					...
					let removedReviewers: string[] = isTeam(existing.reviewer)
					...
					if (
						!newReviewers.find(
							(newTeamReviewer) =>
								newTeamReviewer === existing.reviewer.id,
						)
					) {
				...
				await this._item.requestReview(
					newUserReviewers,
					newTeamReviewers,
				);
				...
				const addedReviewers: ReviewState[] = allReviewers.map(
					(selected) => {
						return {
	...
	private async addMilestone(message: IRequestMessage<void>): Promise<void> {
		return getMilestoneFromQuickPick(
			...
			this._item.milestone,
			(milestone) => this.updateMilestone(milestone, message),
		);
	}
	...
	private async changeProjects(
		message: IRequestMessage<void>,
	): Promise<void> {
		return getProjectFromQuickPick(
			this._folderRepositoryManager,
			this._item.githubRepository,
			this._item.item.projectItems?.map((item) => item.project),
			(project) => this.updateProjects(project, message),
		);
	}
	...
	private async removeProject(
		message: IRequestMessage<IProjectItem>,
	): Promise<void> {
	...
	private async changeAssignees(
		...
		try {
			...
			if (allAssignees) {
				const newAssignees: IAccount[] = allAssignees.map(
					(item) => item.user,
				);

				const removeAssignees: IAccount[] =
					this._item.assignees?.filter(
						(currentAssignee) =>
							!newAssignees.find(
								(newAssignee) =>
	...
	private async addAssigneeYourself(
		message: IRequestMessage<void>,
	): Promise<void> {
		try {
			const currentUser =
			...
			const alreadyAssigned = this._item.assignees?.find(
	...
	private async applyPatch(
		message: IRequestMessage<{ comment: IComment }>,
	): Promise<void> {
		try {
	...
	private async openDiff(
		...
		try {
			...
			return PullRequestModel.openDiffFromComment(
				this._folderRepositoryManager,
				this._item,
				comment,
			);
			...
			Logger.error(
				`Open diff view failed: ${formatError(e)}`,
				PullRequestOverviewPanel.ID,
			);
		}
	}
	...
	private mergePullRequest(message: IRequestMessage<MergeArguments>): void {
		...
		this._folderRepositoryManager
			...
			.then((result) => {
				...
				if (!result.merged) {
					vscode.window.showErrorMessage(
						`Merging PR failed: ${result.message}`,
					);
				}
	...
	private updateReviewers(review?: CommonReviewEvent): void {
		if (review && review.state) {
			const existingReviewer = this._existingReviewers.find(
				(reviewer) =>
					review.user.login === (reviewer.reviewer as IAccount).login,
			);
	...
	private async revert(message: IRequestMessage<string>): Promise<void> {
		await this._folderRepositoryManager.createPullRequestHelper.revert(
			this._telemetry,
			this._extensionUri,
			this._folderRepositoryManager,
			this._item,
	...
	private async updateAutoMerge(
		...
		if (!message.args.autoMerge && !this._item.autoMerge) {
			...
			if (
				this._item.autoMerge &&
				message.args.autoMergeMethod !== this._item.autoMergeMethod
			) {
				await this._item.disableAutoMerge();
			}
	...
	protected override editCommentPromise(
		comment: IComment,
		text: string,
	): Promise<IComment> {
...
export function getDefaultMergeMethod(
	...
	if (
		userPreferred &&
		methodsAvailability.hasOwnProperty(userPreferred) &&
		methodsAvailability[userPreferred]
	) {
