	...
	implements vscode.TerminalLinkProvider
{
	...
	static registerProvider(
		...
		return vscode.window.registerTerminalLinkProvider(
			new GitHubCreatePullRequestLinkProvider(
				reviewManager,
				folderManager,
			),
		);
	...
	provideTerminalLinks(
		...
		if (result && result.length === 4) {
			...
			const hasMatchingGitHubRepo =
				this.folderRepositoryManager.gitHubRepositories.findIndex(
					(repo) =>
						...
						repo.remote.repositoryName === repositoryName,
				) > -1;
			...
			if (
				...
				this.reviewManager.repository.state.HEAD?.name === branchName
			) {
				return [
					{
						startIndex,
	...
	handleTerminalLink(
		link: GitHubCreateTerminalLink,
	): vscode.ProviderResult<void> {
		const defaultHandler =
			GitHubCreatePullRequestLinkProvider.getSettingsValue();
