...
export class ConflictResolutionModel {
	...
	constructor(
		public readonly startingConflicts: Conflict[],
		public readonly repositoryName: string,
		public readonly prBaseOwner: string,
		public readonly latestPrBaseSha: string,
		public readonly prHeadOwner: string,
		public readonly prHeadBranchName: string,
		public readonly prBaseBranchName: string,
		public readonly prMergeBaseRef: string,
	) {
	...
	isResolvable(): boolean {
		return Array.from(this._startingConflicts.values()).every(
			(conflict) => {
	...
	addResolution(filePath: string, contents: string): void {
		this._resolvedConflicts.set(filePath, {
	...
	public mergeOutputUri(conflict: Conflict) {
		return vscode.Uri.parse(
			`${this.mergeScheme}:/${conflict.prHeadFilePath}`,
		);
	}
	...
	public mergeBaseUri(conflict: { prHeadFilePath: string }): vscode.Uri {
		...
		return toGitHubUri(fileUri, Schemes.GithubPr, {
			fileName: conflict.prHeadFilePath,
			branch: this.prMergeBaseRef,
			owner: this.prBaseOwner,
		});
	}
	...
	public prHeadUri(conflict: Conflict): vscode.Uri {
		...
		return toGitHubUri(fileUri, Schemes.GithubPr, {
			fileName: conflict.prHeadFilePath,
			branch: this.prHeadBranchName,
			owner: this.prHeadOwner,
		});
	}
}
