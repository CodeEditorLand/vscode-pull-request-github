...
import {
	getAssigneesQuickPickItems,
	getLabelOptions,
	getMilestoneFromQuickPick,
	getProjectFromQuickPick,
	reviewersQuickPick,
} from "./quickPicks";
import {
	getIssueNumberLabelFromParsed,
	ISSUE_EXPRESSION,
...
const ISSUE_CLOSING_KEYWORDS = new RegExp(
	"closes|closed|close|fixes|fixed|fix|resolves|resolved|resolve\s$",
	"i",
...
export abstract class BaseCreatePullRequestViewProvider<
		T extends BasePullRequestDataModel = BasePullRequestDataModel,
	>
	...
	implements vscode.WebviewViewProvider
{
	...
	readonly onDone: vscode.Event<PullRequestModel | undefined> =
		this._onDone.event;
	...
	public static withProgress<R>(
		...
		return vscode.window.withProgress(
			{ location: { viewId: "github:createPullRequestWebview" } },
			task,
		);
	}

	protected async getPullRequestDefaultLabels(
		defaultBaseRemote: RemoteInfo,
	): Promise<ILabel[]> {
		const pullRequestLabelSettings = vscode.workspace
			.getConfiguration(PR_SETTINGS_NAMESPACE)
			.inspect<string[]>(PULL_REQUEST_LABELS);
		...
		const labels = await this._folderRepositoryManager.getLabels(
			undefined,
			{
				owner: defaultBaseRemote.owner,
				repo: defaultBaseRemote.repositoryName,
			},
		);
		const defaultLabels = labels.filter((label) =>
			defaultLabelValues.includes(label.name),
		);

		return defaultLabels;
	}

	protected abstract getTitleAndDescription(
		compareBranch: Branch,
		baseBranch: string,
	): Promise<{ title: string; description: string }>;

	protected async getMergeConfiguration(
		owner: string,
		name: string,
		refetch: boolean = false,
	): Promise<RepoAccessAndMergeMethods> {
		const repo =
			await this._folderRepositoryManager.createGitHubRepositoryFromOwnerName(
				owner,
				name,
			);
	...
	protected abstract detectBaseMetadata(
		defaultCompareBranch: Branch,
	): Promise<BaseBranchMetadata | undefined>;

	protected getTitleAndDescriptionProvider(name?: string) {
		return this._folderRepositoryManager.getTitleAndDescriptionProvider(
			name,
		);
	}

	protected async getCreateParams(): Promise<CreateParamsNew> {
		const defaultCompareBranch =
			await this._folderRepositoryManager.repository.getBranch(
				this._defaultCompareBranch,
			);
		const [detectedBaseMetadata, remotes, defaultOrigin] =
			await Promise.all([
		...
		const defaultBaseRemote: RemoteInfo = {
			owner:
				detectedBaseMetadata?.owner ?? this._pullRequestDefaults.owner,
			repositoryName:
				detectedBaseMetadata?.repositoryName ??
				this._pullRequestDefaults.repo,
		};
		...
		const defaultBaseBranch =
			detectedBaseMetadata?.branch ?? this._pullRequestDefaults.base;
		...
		const [
			...
			this.getMergeConfiguration(
				defaultBaseRemote.owner,
				defaultBaseRemote.repositoryName,
			),
			defaultOrigin.getViewerPermission(),
			this._folderRepositoryManager.mergeQueueMethodForBranch(
				defaultBaseBranch,
				defaultBaseRemote.owner,
				defaultBaseRemote.repositoryName,
			),
		...
		const defaultCreateOption = vscode.workspace
			.getConfiguration(PR_SETTINGS_NAMESPACE)
			.get<
				"lastUsed" | "create" | "createDraft" | "createAutoMerge"
			>(DEFAULT_CREATE_OPTION, "lastUsed");
		const lastCreateMethod:
			| {
					autoMerge: boolean;
			...
			| undefined =
			this._folderRepositoryManager.context.workspaceState.get<
				| { autoMerge: boolean; mergeMethod: MergeMethod; isDraft }
				| undefined
			>(PREVIOUS_CREATE_METHOD, undefined);
		const repoMergeMethod = getDefaultMergeMethod(
			mergeConfiguration.mergeMethodsAvailability,
		);
		...
		defaultMergeMethod =
			defaultCreateOption === "lastUsed" && lastCreateMethod?.mergeMethod
				? lastCreateMethod?.mergeMethod
				: repoMergeMethod;

		if (defaultCreateOption === "lastUsed") {
			defaultMergeMethod =
				lastCreateMethod?.mergeMethod ?? repoMergeMethod;
			isDraftDefault = !!lastCreateMethod?.isDraft;
			autoMergeDefault =
				mergeConfiguration.viewerCanAutoMerge &&
				!!lastCreateMethod?.autoMerge;
		...
		const useCopilot: boolean =
			!!this.getTitleAndDescriptionProvider("Copilot") &&
			vscode.workspace
				.getConfiguration(PR_SETTINGS_NAMESPACE)
				.get<
					"commit" | "template" | "none" | "Copilot"
				>(PULL_REQUEST_DESCRIPTION) === "Copilot";
		const defaultTitleAndDescriptionProvider =
			this.getTitleAndDescriptionProvider()?.title;
		if (defaultTitleAndDescriptionProvider) {
			...
			*/
			this.telemetry.sendTelemetryEvent(
				"pr.defaultTitleAndDescriptionProvider",
				{ providerTitle: defaultTitleAndDescriptionProvider },
			);
		}
		...
		const params: CreateParamsNew = {
			...
			generateTitleAndDescriptionTitle:
				defaultTitleAndDescriptionProvider,
			creating: false,
	...
	private async doInitializeParams(): Promise<CreateParamsNew> {
		...
		Logger.appendLine(
			`Initializing "create" view: ${JSON.stringify(params)}`,
			BaseCreatePullRequestViewProvider.ID,
		);
	...
	private async autoAssign(pr: PullRequestModel): Promise<void> {
		const configuration = vscode.workspace
			.getConfiguration(PR_SETTINGS_NAMESPACE)
			.get<string | undefined>(ASSIGN_TO);
		if (!configuration) {
			return;
		}
		const resolved = await variableSubstitution(
			configuration,
			pr,
			undefined,
			(
				await this._folderRepositoryManager.getCurrentUser(
					pr.githubRepository,
				)
			)?.login,
		);
	...
	private async enableAutoMerge(
		pr: PullRequestModel,
		autoMerge: boolean,
		automergeMethod: MergeMethod | undefined,
	): Promise<void> {
	...
	private async getBaseRemote(): Promise<GitHubRemote> {
		return (await this._folderRepositoryManager.getGitHubRemotes()).find(
			(remote) =>
				compareIgnoreCase(remote.owner, this.model.baseOwner) === 0 &&
				compareIgnoreCase(
	...
	private getBaseGitHubRepo(): GitHubRepository | undefined {
		return this._folderRepositoryManager.gitHubRepositories.find(
			(repo) =>
				compareIgnoreCase(repo.remote.owner, this.model.baseOwner) ===
					0 &&
				compareIgnoreCase(
	...
	public async addMilestone(): Promise<void> {
		...
		const repo = this._folderRepositoryManager.gitHubRepositories.find(
			(repo) => repo.remote.remoteName === remote.remoteName,
		)!;

		return getMilestoneFromQuickPick(
	...
	public async addReviewers(): Promise<void> {
		...
		try {
			...
			const [metadata, author, teamsCount] = await Promise.all([
				repo?.getMetadata(),
				this._folderRepositoryManager.getCurrentUser(),
				this._folderRepositoryManager.getOrgTeamsCount(repo),
			]);
			quickPick = await reviewersQuickPick(
				...
				this.reviewers.map((reviewer) => {
					return { reviewer, state: "REQUESTED" };
				}),
				[],
			);
			...
			const acceptPromise = asPromise<void>(quickPick.onDidAccept).then(
				() => {
					return quickPick!.selectedItems.filter(
						(item) => item.user,
					) as
						| (vscode.QuickPickItem & { user: IAccount | ITeam })[]
						| undefined;
				},
			);
	...
	public async addAssignees(): Promise<void> {
		const remote = await this.getBaseRemote();
		const currentRepo =
			this._folderRepositoryManager.gitHubRepositories.find(
				(repo) =>
					repo.remote.owner === remote.owner &&
					repo.remote.repositoryName === remote.repositoryName,
			);
		...
		if (assigneesToAdd) {
			...
			const addedAssignees = assigneesToAdd
				.map((assignee) => assignee.user)
				.filter<IAccount>((assignee): assignee is IAccount => {
	...
	private async removeLabel(
		message: IRequestMessage<{ label: ILabel }>,
	): Promise<void> {
		...
		if (previousLabelsLength === this.labels.length) return;
	...
	public async createFromCommand(
		isDraft: boolean,
		autoMerge: boolean,
		autoMergeMethod: MergeMethod | undefined,
	...
	protected async postCreate(
		message: IRequestMessage<CreatePullRequestNew>,
		createdPR: PullRequestModel,
	) {
		return Promise.all([
			this.setLabels(createdPR, message.args.labels),
			this.enableAutoMerge(
				createdPR,
				message.args.autoMerge,
				message.args.autoMergeMethod,
			),
	...
	private async cancel(message: IRequestMessage<CreatePullRequestNew>) {
		...
		await this.getMergeConfiguration(
			message.args.owner,
			message.args.repo,
			true,
		);
...
function serializeRemoteInfo(remote: {
	owner: string;
	repositoryName: string;
}) {
...
export class CreatePullRequestViewProvider
	extends BaseCreatePullRequestViewProvider<CreatePullRequestDataModel>
	implements vscode.WebviewViewProvider
{
	public override readonly viewType = "github:createPullRequestWebview";
	...
	constructor(
		...
		this._register(
			this.model.onDidChange(async (e) => {
				let baseRemote: RemoteInfo | undefined;
				let baseBranch: string | undefined;
				if (e.baseOwner) {
					const gitHubRemote = this._folderRepositoryManager.findRepo(
						(repo) =>
							compareIgnoreCase(
								repo.remote.owner,
								e.baseOwner!,
					...
					baseRemote = gitHubRemote
						? serializeRemoteInfo(gitHubRemote)
						: undefined;
	...
	public async setDefaultCompareBranch(compareBranch: Branch | undefined) {
		const branchChanged =
			compareBranch && compareBranch.name !== this.model.compareBranch;
		const currentCompareRemote =
			this._folderRepositoryManager.gitHubRepositories.find(
				(repo) => repo.remote.owner === this.model.compareOwner,
			)?.remote.remoteName;
		const branchRemoteChanged =
			compareBranch &&
			compareBranch.upstream?.remote !== currentCompareRemote;
		if (branchChanged || branchRemoteChanged) {
			...
			this.changeBranch(compareBranch!.name!, false).then(
				(titleAndDescription) => {
					const params: Partial<CreateParamsNew> = {
	...
	private async getTotalGitHubCommits(
		compareBranch: Branch,
		baseBranchName: string,
	): Promise<
		| { commit: { message: string }; parents: { sha: string }[] }[]
		| undefined
	> {
		const origin =
			await this._folderRepositoryManager.getOrigin(compareBranch);

		if (compareBranch.upstream) {
			const headRepo = this._folderRepositoryManager.findRepo(
				byRemoteName(compareBranch.upstream.remote),
			);

			if (headRepo) {
				const headBranch = `${headRepo.remote.owner}:${compareBranch.name ?? ""}`;
				const baseBranch = `${this._pullRequestDefaults.owner}:${baseBranchName}`;
				const compareResult = await origin.compareCommits(
					baseBranch,
					headBranch,
				);
	...
	protected async getTitleAndDescription(
		compareBranch: Branch,
		baseBranch: string,
	): Promise<{ title: string; description: string }> {
		let title: string = "";
		let description: string = "";
		const descrptionSource = vscode.workspace
			.getConfiguration(PR_SETTINGS_NAMESPACE)
			.get<
				"commit" | "template" | "none" | "Copilot"
			>(PULL_REQUEST_DESCRIPTION);
		...
		let useBranchName =
			this._pullRequestDefaults.base === compareBranch.name;
		Logger.debug(
			`Compare branch name: ${compareBranch.name}, Base branch name: ${this._pullRequestDefaults.base}`,
			CreatePullRequestViewProvider.ID,
		);
		try {
			const name = compareBranch.name;
			const [totalCommits, lastCommit, pullRequestTemplate] =
				await Promise.all([
					this.getTotalGitHubCommits(compareBranch, baseBranch),
					name
						? titleAndBodyFrom(
								promiseWithTimeout(
									this._folderRepositoryManager.getTipCommitMessage(
										name,
									),
									5000,
								),
						...
						: undefined,
					descrptionSource === "template"
						? await this.getPullRequestTemplate()
						: undefined,
				]);
			const totalNonMergeCommits = totalCommits?.filter(
				(commit) => commit.parents.length < 2,
			);

			Logger.debug(
				`Total commits: ${totalNonMergeCommits?.length}`,
				CreatePullRequestViewProvider.ID,
			);
			...
			if (name && !lastCommit) {
				Logger.appendLine(
					"Timeout getting last commit message",
					CreatePullRequestViewProvider.ID,
				);
				/* __GDPR__
			...
			if (!description) {
				...
				if (match?.issueNumber && !match.name && !match.owner) {
					...
					const prefix = title.substr(
						0,
						title.indexOf(issueExpMatch![0]),
					);
			...
			Logger.debug(
				`Error while getting total commits: ${e}`,
				CreatePullRequestViewProvider.ID,
			);
		}
		return { title, description };
	...
	private async getPullRequestTemplate(): Promise<string | undefined> {
		return this._folderRepositoryManager.getPullRequestTemplateBody(
			this.model.baseOwner,
		);
	}

	protected async detectBaseMetadata(
		defaultCompareBranch: Branch,
	): Promise<BaseBranchMetadata | undefined> {
		...
		const settingValue = vscode.workspace
			.getConfiguration(PR_SETTINGS_NAMESPACE)
			.get<
				"repositoryDefault" | "createdFromBranch" | "auto"
			>(CREATE_BASE_BRANCH);
		if (
			!defaultCompareBranch.name ||
			settingValue === "repositoryDefault"
		) {
			return undefined;
		}
		const githubRepo = this._folderRepositoryManager.findRepo(
			(repo) =>
				compareIgnoreCase(repo.remote.owner, owner) === 0 &&
				compareIgnoreCase(
					repo.remote.repositoryName,
					repositoryName,
				) === 0,
		...
		if (
			settingValue === "auto" &&
			(await githubRepo?.getMetadata())?.fork
		) {
			return undefined;
		}

		try {
			const baseFromProvider =
				await this._folderRepositoryManager.repository.getBranchBase(
					defaultCompareBranch.name,
				);
			if (baseFromProvider?.name) {
				const repo = this._folderRepositoryManager.findRepo(
					(repo) =>
						repo.remote.remoteName === baseFromProvider.remote,
				);
				if (repo) {
					return {
	...
	private async remotePicks(
		isBase: boolean,
	): Promise<(vscode.QuickPickItem & { remote?: RemoteInfo })[]> {
		const remotes = isBase
			? await this._folderRepositoryManager.getActiveGitHubRemotes(
					await this._folderRepositoryManager.getGitHubRemotes(),
				)
			: this._folderRepositoryManager.gitHubRepositories.map(
					(repo) => repo.remote,
				);
	...
	private async branchPicks(
		githubRepository: GitHubRepository,
		changeRepoMessage: string,
		isBase: boolean,
	): Promise<
		(vscode.QuickPickItem & { remote?: RemoteInfo; branch?: string })[]
	> {
		let branches: (string | Ref)[];
		if (isBase) {
			// For the base, we only want to show branches from GitHub.
			branches = await githubRepository.listBranches(
			...
			branches = (
				await this._folderRepositoryManager.repository.getBranches({
					remote: false,
				})
		...
		const branchPicks: (vscode.QuickPickItem & {
			remote?: RemoteInfo;
			branch?: string;
		})[] = branches.map((branch) => {
			const branchName =
	...
	private async processRemoteAndBranchResult(
		githubRepository: GitHubRepository,
		result: { remote: RemoteInfo; branch: string },
		isBase: boolean,
	) {
		const [defaultBranch, viewerPermission] = await Promise.all([
			githubRepository.getDefaultBranch(),
			githubRepository.getViewerPermission(),
		]);
		...
		let chooseResult:
			...
			| ChooseCompareRemoteAndBranchResult;
		if (isBase) {
			const baseRemoteChanged =
				this.model.baseOwner !== result.remote.owner;
			const baseBranchChanged =
				baseRemoteChanged || this.model.baseBranch !== result.branch;
			this.model.baseOwner = result.remote.owner;
			...
			if (mergeConfiguration.viewerCanAutoMerge) {
				...
				const lastCreateMethod:
					...
					>(PREVIOUS_CREATE_METHOD, undefined);
				autoMergeDefault =
					(defaultCreateOption === "lastUsed" &&
						lastCreateMethod?.autoMerge) ||
					defaultCreateOption === "createAutoMerge";
			}
			...
			if (baseRemoteChanged) {
				...
				*/
				this._folderRepositoryManager.telemetry.sendTelemetryEvent(
					"pr.create.changedBaseRemote",
				);
			}
	...
	private async changeRemoteAndBranch(
		message: IRequestMessage<ChooseRemoteAndBranchArgs>,
		isBase: boolean,
	): Promise<void> {
		...
		let githubRepository = this._folderRepositoryManager.findRepo(
			(repo) =>
				message.args.currentRemote?.owner === repo.remote.owner &&
				message.args.currentRemote.repositoryName ===
					repo.remote.repositoryName,
		);

		const chooseDifferentRemote = vscode.l10n.t("Change Repository...");
		const remotePlaceholder = vscode.l10n.t("Choose a remote");
		const branchPlaceholder = isBase
			? vscode.l10n.t("Choose a base branch")
			: vscode.l10n.t("Choose a branch to merge");
		const repositoryPlaceholder = isBase
			? vscode.l10n.t("Choose a base repository")
			: vscode.l10n.t("Choose a repository to merge from");

		quickPick.placeholder = githubRepository
			? branchPlaceholder
			: remotePlaceholder;
		quickPick.show();
		quickPick.busy = true;
		quickPick.items = githubRepository
			? await this.branchPicks(
					githubRepository,
					chooseDifferentRemote,
					isBase,
				)
			: await this.remotePicks(isBase);
		const activeItem = message.args.currentBranch
			? quickPick.items.find(
					(item) => item.branch === message.args.currentBranch,
				)
			: undefined;
		quickPick.activeItems = activeItem ? [activeItem] : [];
		quickPick.busy = false;
		const remoteAndBranch: Promise<
			{ remote: RemoteInfo; branch: string } | undefined
		> = new Promise((resolve) => {
			quickPick.onDidAccept(async () => {
				...
				if (selectedPick.label === chooseDifferentRemote) {
					...
					selectedPick.branch === undefined &&
					selectedPick.remote
				) {
					const selectedRemote =
						selectedPick as vscode.QuickPickItem & {
							remote: RemoteInfo;
						};
					quickPick.busy = true;
					githubRepository = this._folderRepositoryManager.findRepo(
						(repo) =>
							repo.remote.owner === selectedRemote.remote.owner &&
							repo.remote.repositoryName ===
								selectedRemote.remote.repositoryName,
					)!;
					quickPick.items = await this.branchPicks(
						githubRepository,
						chooseDifferentRemote,
						isBase,
					);
		...
		quickPick.busy = true;
		const chooseResult = await this.processRemoteAndBranchResult(
			githubRepository,
			result,
			isBase,
		);
	...
	private async findIssueContext(
		commits: string[],
	): Promise<{ content: string; reference: string }[] | undefined> {
		const issues: Promise<
			{ content: string; reference: string } | undefined
		>[] = [];
		for (const commit of commits) {
			const tryParse = parseIssueExpressionOutput(
				commit.match(ISSUE_OR_URL_EXPRESSION),
			);
			if (tryParse) {
				...
				issues.push(
					new Promise((resolve) => {
						this._folderRepositoryManager
							.resolveIssue(owner, name, tryParse.issueNumber)
							.then((issue) => {
								if (issue) {
									resolve({
										content: `${issue.title}\n${issue.body}`,
		...
		if (issues.length) {
			return (await Promise.all(issues)).filter((issue) => !!issue) as {
				content: string;
				reference: string;
			}[];
		}
	...
	private async getCommitsAndPatches(): Promise<{
		commitMessages: string[];
		patches: { patch: string; fileUri: string; previousFileUri?: string }[];
	}> {
		let commitMessages: string[];
		let patches: (
			| { patch: string; fileUri: string; previousFileUri?: string }
			| undefined
		)[];
		if (await this.model.getCompareHasUpstream()) {
			[commitMessages, patches] = await Promise.all([
				this.model
					.gitHubCommits()
					.then((rawCommits) =>
						rawCommits.map((commit) => commit.commit.message),
					),
				this.model.gitHubFiles().then((rawPatches) =>
					rawPatches.map((file) => {
						if (!file.patch) {
							return;
						}
			...
			[commitMessages, patches] = await Promise.all([
				...
				Promise.all(
					(await this.model.gitFiles()).map(async (file) => {
						return {
							patch: await this._folderRepositoryManager.repository.diffBetween(
								this.model.baseBranch,
								this.model.compareBranch,
								file.uri.fsPath,
							),
	...
	private lastGeneratedTitleAndDescription:
		| { title?: string; description?: string; providerTitle: string }
		| undefined;
	private async getTitleAndDescriptionFromProvider(
		token: vscode.CancellationToken,
		searchTerm?: string,
	) {
		return CreatePullRequestViewProvider.withProgress(async () => {
			try {
				const { commitMessages, patches } =
					await this.getCommitsAndPatches();
				const issues = await this.findIssueContext(commitMessages);

				const provider =
					this._folderRepositoryManager.getTitleAndDescriptionProvider(
						searchTerm,
					);
				const result =
					await provider?.provider.provideTitleAndDescription(
						{ commitMessages, patches, issues },
						token,
					);

				if (provider) {
					this.lastGeneratedTitleAndDescription = {
						...result,
				...
				Logger.error(
					`Error while generating title and description: ${e}`,
					CreatePullRequestViewProvider.ID,
				);
				return undefined;
			}
	...
	private generatingCancellationToken:
		| vscode.CancellationTokenSource
		| undefined;
	private async generateTitleAndDescription(
		message: IRequestMessage<TitleAndDescriptionArgs>,
	): Promise<void> {
		...
		const result = await Promise.race([
			this.getTitleAndDescriptionFromProvider(
				this.generatingCancellationToken.token,
				message.args.useCopilot ? "Copilot" : undefined,
			),
			new Promise<true>((resolve) =>
				this.generatingCancellationToken?.token.onCancellationRequested(
					() => resolve(true),
				),
			),
		]);
	...
	private async getPreReviewFromProvider(
		token: vscode.CancellationToken,
	): Promise<PreReviewState | undefined> {
		const preReviewer = this._folderRepositoryManager.getAutoReviewer();
		if (!preReviewer) {
			return;
		}
		const { commitMessages, patches } = await this.getCommitsAndPatches();
		const result = await preReviewer.provider.provideReviewerComments(
			{
				repositoryRoot:
					this._folderRepositoryManager.repository.rootUri.fsPath,
				commitMessages,
				patches,
			},
			token,
		);
		return result && result.succeeded && result.files.length > 0
			? PreReviewState.ReviewedWithComments
			: PreReviewState.ReviewedWithoutComments;
	}
	...
	private reviewingCancellationToken:
		| vscode.CancellationTokenSource
		| undefined;
	private async preReview(message: IRequestMessage<any>): Promise<void> {
		return CreatePullRequestViewProvider.withProgress(async () => {
			...
			this.reviewingCancellationToken =
				new vscode.CancellationTokenSource();

			const result = await Promise.race([
				this.getPreReviewFromProvider(
					this.reviewingCancellationToken.token,
				),
				new Promise<void>((resolve) =>
					this.reviewingCancellationToken?.token.onCancellationRequested(
						() => resolve(),
					),
				),
			]);
	...
	private async pushUpstream(
		compareOwner: string,
		compareRepositoryName: string,
		compareBranchName: string,
	): Promise<
		| { compareUpstream: GitHubRemote; repo: GitHubRepository | undefined }
		| undefined
	> {
		let createdPushRemote: GitHubRemote | undefined;
		const pushRemote =
			this._folderRepositoryManager.repository.state.remotes.find(
				(localRemote) => {
					if (!localRemote.pushUrl) {
						return false;
					}
					const testRemote = new GitHubRemote(
						localRemote.name,
						localRemote.pushUrl,
						new Protocol(localRemote.pushUrl),
						GitHubServerType.GitHubDotCom,
					);
					if (
						testRemote.owner.toLowerCase() ===
							compareOwner.toLowerCase() &&
						testRemote.repositoryName.toLowerCase() ===
							compareRepositoryName.toLowerCase()
					) {
		...
		if (pushRemote && createdPushRemote) {
			Logger.appendLine(
				`Found push remote ${pushRemote.name} for ${compareOwner}/${compareRepositoryName} and branch ${compareBranchName}`,
				CreatePullRequestViewProvider.ID,
			);
			...
			return {
				compareUpstream: createdPushRemote,
				repo: this._folderRepositoryManager.findRepo(
					byRemoteName(createdPushRemote.remoteName),
				),
			};
		}
	}
	...
	private checkGeneratedTitleAndDescription(
		...
		const usedGeneratedTitle: boolean =
			!!this.lastGeneratedTitleAndDescription.title &&
			(this.lastGeneratedTitleAndDescription.title === title ||
				this.lastGeneratedTitleAndDescription.title?.includes(title) ||
				title?.includes(this.lastGeneratedTitleAndDescription.title));
		const usedGeneratedDescription: boolean =
			!!this.lastGeneratedTitleAndDescription.description &&
			(this.lastGeneratedTitleAndDescription.description ===
				description ||
				this.lastGeneratedTitleAndDescription.description?.includes(
					description,
				) ||
		...
		*/
		this.telemetry.sendTelemetryEvent(
			...
			{
				providerTitle:
					this.lastGeneratedTitleAndDescription.providerTitle,
				usedGeneratedTitle: usedGeneratedTitle.toString(),
				usedGeneratedDescription: usedGeneratedDescription.toString(),
			},
		);
	}
	...
	private async checkForChanges(): Promise<boolean> {
		if (await this.model.filesHaveChanges()) {
			...
			const result = await vscode.window.showWarningMessage(
				vscode.l10n.t(
					"You have made changes to the files in this pull request. Do you want to commit these changes to the pull request before creating it?",
				),
				{ modal: true },
				apply,
				deleteChanges,
			...
			if (result === apply) {
				const commitMessage = await vscode.window.showInputBox({
					prompt: vscode.l10n.t("Commit message for your changes"),
				});
				if (commitMessage) {
	...
	protected async create(
		...
		Logger.debug(
			`Creating pull request with args ${JSON.stringify(message.args)}`,
			CreatePullRequestViewProvider.ID,
		);

		if (!(await this.checkForChanges())) {
			Logger.debug(
				"Not continuing past checking for file changes.",
				CreatePullRequestViewProvider.ID,
			);
		...
		const createMethod: {
			...
			autoMerge: message.args.autoMerge,
			mergeMethod: message.args.autoMergeMethod,
			isDraft: message.args.draft,
		};
		this._folderRepositoryManager.context.workspaceState.update(
			PREVIOUS_CREATE_METHOD,
			createMethod,
		);
		...
		CreatePullRequestViewProvider.withProgress(() => {
			return vscode.window.withProgress(
				...
				async (progress) => {
					...
					progress.report({
						message: vscode.l10n.t("Checking for upstream branch"),
					...
					try {
						...
						let headRepo = compareBranch.upstream
							? this._folderRepositoryManager.findRepo(
									(githubRepo) => {
										return (
											githubRepo.remote.owner ===
												compareOwner &&
											githubRepo.remote.repositoryName ===
												compareRepositoryName
										);
									},
								)
							: undefined;
						...
						if (
							...
							const pushBranchSetting =
								vscode.workspace
									.getConfiguration(PR_SETTINGS_NAMESPACE)
									.get(PUSH_BRANCH) === "always";
							const messageResult = !pushBranchSetting
								? await vscode.window.showInformationMessage(
										vscode.l10n.t(
											"There is no remote branch on {0}/{1} for '{2}'.\n\nDo you want to publish it and then create the pull request?",
											compareOwner,
											compareRepositoryName,
											compareBranchName,
										),
							...
							if (
								messageResult === alwaysPublish ||
								messageResult === publish
							) {
								progress.report({
									message: vscode.l10n.t("Pushing branch"),
									increment: 10,
								});
								totalIncrement += 10;

								const pushResult = await this.pushUpstream(
									compareOwner,
									compareRepositoryName,
									compareBranchName,
								);
								if (pushResult) {
									existingCompareUpstream =
										pushResult.compareUpstream;
									headRepo = pushResult.repo;
								} else {
									this._throwError(
										message,
										vscode.l10n.t(
											"The current repository does not have a push remote for {0}",
											compareGithubRemoteName,
										),
									);
								}
							}
						...
						if (!existingCompareUpstream) {
							this._throwError(
								message,
								vscode.l10n.t(
									"No remote branch on {0}/{1} for the merge branch.",
									compareOwner,
									compareRepositoryName,
								),
							);
						...
						if (!headRepo) {
							throw new Error(
								vscode.l10n.t(
									"Unable to find GitHub repository matching '{0}'. You can add '{0}' to the setting \"githubPullRequests.remotes\" to ensure '{0}' is found.",
									existingCompareUpstream.remoteName,
								),
							);
						}

						progress.report({
							message: vscode.l10n.t("Creating pull request"),
							increment: 70 - totalIncrement,
						});
						totalIncrement += 70 - totalIncrement;
						const head = `${headRepo.remote.owner}:${compareBranchName}`;
						this.checkGeneratedTitleAndDescription(
							message.args.title,
							message.args.body,
						);
						createdPR =
							await this._folderRepositoryManager.createPullRequest(
								{ ...message.args, head },
							);
						...
						if (!createdPR) {
							this._throwError(
								message,
								vscode.l10n.t(
									"There must be a difference in commits to create a pull request.",
								),
							);
						} else {
						...
						if (!createdPR) {
							...
							vscode.window.showErrorMessage(
								vscode.l10n.t(
									"There was an error creating the pull request: {0}",
									(e as Error).message,
								),
							);
						}
	...
	private async changeBranch(
		...
		if (isBase) {
			...
			try {
				...
				vscode.window.showErrorMessage(
					vscode.l10n.t("Branch does not exist locally."),
				);
			}
		}
		...
		return this.getTitleAndDescription(
			compareBranch,
			this.model.baseBranch,
		);
	}
