...
const LAST_USED_SCOPES_ENTERPRISE_KEY =
	"githubPullRequest.lastUsedScopesEnterprise";
...
export class CredentialStore extends Disposable {
	...
	constructor(
		...
		this._register(
			vscode.authentication.onDidChangeSessions((e) =>
				this.handlOnDidChangeSessions(e),
			),
		);
	}

	private async handlOnDidChangeSessions(
		e: vscode.AuthenticationSessionsChangeEvent,
	) {
		const currentProvider =
			e.provider.id === AuthProvider.github && this._githubAPI
				? AuthProvider.github
				: e.provider.id === AuthProvider.githubEnterprise &&
					  this._githubEnterpriseAPI
					? AuthProvider.githubEnterprise
					: undefined;

		if (
			(this._githubAPI || this._githubEnterpriseAPI) &&
			!currentProvider
		) {
			return;
		}
		if (currentProvider) {
			const newSession = await this.getSession(
				currentProvider,
				{ silent: true },
				currentProvider === AuthProvider.github
					? this._scopes
		...
		if (
			!this.isAuthenticated(AuthProvider.githubEnterprise) &&
			hasEnterpriseUri()
		) {
	...
	private async initialize(
		...
		scopes: string[] = !isEnterprise(authProviderId)
			? this._scopes
			: this._scopesEnterprise,
		requireScopes?: boolean,
	): Promise<AuthResult> {
		Logger.debug(
			`Initializing GitHub${getGitHubSuffix(authProviderId)} authentication provider.`,
			"Authentication",
		);
		...
		if (isEnterprise(authProviderId)) {
			if (!hasEnterpriseUri()) {
				Logger.debug(
					`GitHub Enterprise provider selected without URI.`,
					"Authentication",
				);
		...
		if (
			getAuthSessionOptions.createIfNone === undefined &&
			getAuthSessionOptions.forceNewSession === undefined
		) {
		...
		if (session) {
			...
			Logger.debug(
				`No GitHub${getGitHubSuffix(authProviderId)} token found.`,
				"Authentication",
			);
	...
	private async doCreate(
		...
		additionalScopes: boolean = false,
	): Promise<AuthResult> {
		const github = await this.initialize(
			AuthProvider.github,
			options,
			additionalScopes ? SCOPES_WITH_ADDITIONAL : undefined,
			additionalScopes,
		);
		...
		if (hasEnterpriseUri()) {
			enterprise = await this.initialize(
				AuthProvider.githubEnterprise,
				options,
				additionalScopes ? SCOPES_WITH_ADDITIONAL : undefined,
				additionalScopes,
	...
	public async recreate(reason?: string): Promise<AuthResult> {
		return this.doCreate({
			forceNewSession: reason ? { detail: reason } : true,
		});
	}
	...
	public isAnyAuthenticated() {
		return (
			this.isAuthenticated(AuthProvider.github) ||
			this.isAuthenticated(AuthProvider.githubEnterprise)
		);
	}
	...
	public isAuthenticatedWithAdditionalScopes(
		authProviderId: AuthProvider,
	): boolean {
		if (!isEnterprise(authProviderId)) {
			return (
	...
	public async showSignInNotification(
		authProviderId: AuthProvider,
	): Promise<GitHub | undefined> {
		if (
			PersistentState.fetch(
				PROMPT_FOR_SIGN_IN_SCOPE,
				PROMPT_FOR_SIGN_IN_STORAGE_KEY,
			) === false
		) {
			return;
		}
		...
		const result = await vscode.window.showInformationMessage(
			vscode.l10n.t(
				"In order to use the Pull Requests functionality, you must sign in to GitHub{0}",
				getGitHubSuffix(authProviderId),
			),
			SIGNIN_COMMAND,
			IGNORE_COMMAND,
		);
	...
	public async login(
		...
		*/
		...
		const errorPrefix = vscode.l10n.t(
			"Error signing in to GitHub{0}",
			getGitHubSuffix(authProviderId),
		);
	...
	public async showSamlMessageAndAuth(
		...
		const result = await this.recreate(
			vscode.l10n.t(
				"GitHub Pull Requests requires that you provide SAML access to your organization ({0}) when you sign in.",
				organizations.join(", "),
			),
		);
	...
	public async isCurrentUser(username: string): Promise<boolean> {
		return (
			(await this._githubAPI?.currentUser)?.login === username ||
			(await this._githubEnterpriseAPI?.currentUser)?.login == username
		);
	}
	...
	private setCurrentUser(github: GitHub): void {
		const getUser: ReturnType<
			typeof github.octokit.api.users.getAuthenticated
		> = new Promise((resolve) => {
			github.octokit
				.call(github.octokit.api.users.getAuthenticated, {})
				.then((result) => {
					resolve(result);
				});
		});
	...
	private async getSession(
		...
		const existingSession =
			getAuthSessionOptions.forceNewSession || requireScopes
				? undefined
				: await this.findExistingScopes(authProviderId);
		...
		const session = await vscode.authentication.getSession(
			authProviderId,
			requireScopes ? scopes : SCOPES_OLD,
			getAuthSessionOptions,
		);
		...
		return {
			session,
			isNew: !!session,
			scopes: requireScopes ? scopes : SCOPES_OLD,
		};
	}

	private async findExistingScopes(
		authProviderId: AuthProvider,
	): Promise<
		{ session: vscode.AuthenticationSession; scopes: string[] } | undefined
	> {
		const scopesInPreferenceOrder = [
			SCOPES_WITH_ADDITIONAL,
			SCOPES_OLD,
			SCOPES_OLDEST,
		];
	...
	private async createHub(
		...
		if (
			enterpriseServerUri &&
			enterpriseServerUri.authority.endsWith("ghe.com")
		) {
		...
		const rateLogger = new RateLogger(
			this._telemetry,
			isEnterprise(authProviderId),
		);
