...
export class RepositoriesManager implements vscode.Disposable {
	...
	async authenticate(enterprise?: boolean): Promise<boolean> {
		...
		if (enterprise) {
			const remoteToUse =
				getEnterpriseUri()?.toString() ??
				(enterpriseRemotes.length
					? enterpriseRemotes[0].normalizedHost
					: unknownRemotes.length
						? unknownRemotes[0].normalizedHost
						: undefined);
			...
			if (remoteToUse) {
				const promptResult = await vscode.window.showInformationMessage(
					vscode.l10n.t(
						"Would you like to set up GitHub Pull Requests and Issues to authenticate with the enterprise server {0}?",
						remoteToUse,
					),
					{ modal: true },
					yes,
				...
				const setEnterpriseUriPrompt = await vscode.window.showInputBox(
					{
						placeHolder: vscode.l10n.t(
							"Set a GitHub Enterprise server URL",
						),
						ignoreFocusOut: true,
					},
				);
