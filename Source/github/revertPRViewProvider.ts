...
export class RevertPullRequestViewProvider
	extends BaseCreatePullRequestViewProvider
	implements vscode.WebviewViewProvider, vscode.Disposable
{
	constructor(
		...
		super(
			...
			folderRepositoryManager,
			pullRequestDefaults,
			folderRepositoryManager.repository.state.HEAD?.name ??
				pullRequest.base.name,
		);
	}
	...
	protected async getTitleAndDescription(): Promise<{
		...
		return {
			...
			description: vscode.l10n.t(
				"Reverts {0}",
				`${this.pullRequest.remote.owner}/${this.pullRequest.remote.repositoryName}#${this.pullRequest.number}`,
			),
		};
	}

	protected async detectBaseMetadata(): Promise<
		BaseBranchMetadata | undefined
	> {
		return {
	...
	protected async create(
		...
		RevertPullRequestViewProvider.withProgress(async () => {
			...
			try {
				revertPr = await this._folderRepositoryManager.revert(
					this.pullRequest,
					message.args.title,
					message.args.body,
					message.args.draft,
