...
function repoResponseAsRemoteSource(
	raw: OctokitCommon.SearchReposResponseItem,
): RemoteSource {
	return {
...
export class GithubRemoteSourceProvider implements RemoteSourceProvider {
	...
	constructor(
		...
		private readonly authProviderId: AuthProvider = AuthProvider.github,
	) {
	...
	private async getUserRemoteSources(
		...
		if (!query) {
			const res = await hub.octokit.call(
	...
	private async getQueryRemoteSources(
		...
		const raw = await hub.octokit.call(hub.octokit.api.search.repos, {
			q: query,
			sort: "updated",
		});
