...
export class ReviewsManager {
	...
	constructor(
		...
		this._disposables = [];
		const gitContentProvider = new GitContentFileSystemProvider(
			_gitApi,
			_credentialStore,
	...
	async provideTextDocumentContent(
		...
		for (const reviewManager of this._reviewManagers) {
			if (
				isDescendant(
					reviewManager.repository.rootUri.fsPath,
					uri.fsPath,
				)
			) {
	...
	public addReviewManager(reviewManager: ReviewManager) {
		...
		if (workspaceFolders) {
			const index = workspaceFolders.findIndex(
				(folder) =>
					folder.uri.toString() ===
					reviewManager.repository.rootUri.toString(),
			);
			if (index > -1) {
				const arrayEnd = this._reviewManagers.slice(
					index,
