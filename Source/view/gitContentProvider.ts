...
export class GitContentFileSystemProvider extends RepositoryFileSystemProvider {
	...
	constructor(
		...
		private readonly getReviewManagers: () => ReviewManager[],
	) {
		super(gitAPI, credentialStore);
	}

	private getChangeModelForFileAndFilesArray(
		file: vscode.Uri,
		getFiles: (
			manager: ReviewManager,
		) => (GitFileChangeNode | RemoteFileChangeNode)[],
	) {
		for (const manager of this.getReviewManagers()) {
			const files = getFiles(manager);
			for (const change of files) {
				if (
					change.changeModel.filePath.authority === file.authority &&
					change.changeModel.filePath.path === file.path
				) {
	...
	private getChangeModelForFile(
		file: vscode.Uri,
	): GitFileChangeModel | undefined {
		return this.getChangeModelForFileAndFilesArray(
			file,
			(manager) => manager.reviewModel.localFileChanges,
		) as GitFileChangeModel;
	}

	private getOutdatedChangeModelForFile(file: vscode.Uri) {
		return this.getChangeModelForFileAndFilesArray(
			file,
			(manager) => manager.reviewModel.obsoleteFileChanges,
		);
	}

	private async getRepositoryForFile(
		file: vscode.Uri,
	...
	async readFile(uri: vscode.Uri): Promise<Uint8Array> {
		...
		const repository = await this.getRepositoryForFile(
			vscode.Uri.file(rootPath),
		);
		if (!repository) {
			vscode.window.showErrorMessage(
				`We couldn't find an open repository for ${commit} locally.`,
			);
			return new TextEncoder().encode("");
		}
