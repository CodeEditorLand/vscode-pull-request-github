...
export class ReviewManager {
	...
	private registerListeners(): void {
		this._disposables.push(
			this._repository.state.onDidChange((_) => {
				...
				const sameRemotes =
					this._previousRepositoryState.remotes.length ===
						remotes.length &&
					...
					vscode.window.registerQuickDiffProvider(
						{ scheme: "file" },
						{
							provideOriginalResource: (uri: vscode.Uri) => {
								const changeNode =
									this.reviewModel.localFileChanges.find(
										(changeNode) =>
											changeNode.changeModel.filePath.toString() ===
											uri.toString(),
									);
								if (changeNode) {
									return changeNode.changeModel
										.parentFilePath;
								}
							},
						},
	...
	private async offerIgnoreBranch(currentBranchName): Promise<boolean> {
		...
		if (offerResult === ignore) {
			...
			const setting = settingNamespace.get<string[]>(
				IGNORE_PR_BRANCHES,
				[],
			);
	...
	private async getUpstreamUrlAndName(branch: Branch): Promise<{
		...
		if (branch.upstream) {
			return {
				...
				branchName: branch.upstream.name,
				url: undefined,
			};
		} else {
			try {
				const url = await this.repository.getConfig(
	...
	private async checkGitHubForPrBranch(
		...
		(PullRequestMetadata & { model: PullRequestModel }) | undefined
	> {
		const { url, branchName, remoteName } =
			await this.getUpstreamUrlAndName(this._repository.state.HEAD!);
	...
	private async validateState(silent: boolean, updateLayout: boolean) {
			...
			(branch.remote === "origin" ||
				!(
					await this._folderRepoManager.gitHubRepositories
						.find(
							(repo) => repo.remote.remoteName === branch.remote,
						)
						?.getMetadata()
				)?.fork)
		) {
		...
		if (pr.state !== GithubItemStateEnum.Open) {
			...
			if (
				metadataFromGithub &&
				metadataFromGithub?.prNumber !== pr.number
			) {
				const prFromGitHub =
					await this.resolvePullRequest(metadataFromGithub);
				if (prFromGitHub) {
					pr = prFromGitHub;
				}
			}
	...
	private layout(
		...
		if ((!silent || this._showPullRequest.shouldShow) && isFocusMode) {
			...
			const showPRChangedDisposable =
				this._showPullRequest.onChangedShowValue((shouldShow) => {
					Logger.appendLine(
						`PR show value changed = ${shouldShow}.`,
						this.id,
					);
					if (shouldShow) {
						this._doFocusShow(pr, updateLayout);
	...
	private async reopenNewReviewDiffs() {
					...
					return tabGroup.tabs.map((tab) => {
						if (tab.input instanceof vscode.TabInputTextDiff) {
							if (tab.input.original.scheme === Schemes.Review) {
								for (const localChange of this._reviewModel
									.localFileChanges) {
									const fileName = fromReviewUri(
										tab.input.original.query,
									);
		...
		if (!hasOpenDiff && this._reviewModel.localFileChanges.length) {
			this._reviewModel.localFileChanges[0].openDiff(
				this._folderRepoManager,
				{ preview: true },
			);
		}
		...
		if (this._reviewModel.localFileChanges.length) {
			...
			for (const fileChange of this._reviewModel.localFileChanges) {
				if (fileChange.status === GitChangeType.MODIFY) {
					if (
						KnownMediaExtensions.includes(
							nodePath.extname(fileChange.fileName),
						)
					) {
	...
	private _doFocusShow(pr: PullRequestModel, updateLayout: boolean) {
		...
		if (updateLayout) {
			const focusedMode = vscode.workspace
				.getConfiguration(PR_SETTINGS_NAMESPACE)
				.get<
					"firstDiff" | "overview" | "multiDiff" | false
				>(FOCUSED_MODE);
					...
					return tabGroup.tabs.map((tab) => {
						if (tab.input instanceof vscode.TabInputTextDiff) {
							if (
								...
								const originalParams = fromPRUri(
									tab.input.original,
								);
								const modifiedParams = fromPRUri(
									tab.input.modified,
								);
								if (
									originalParams?.prNumber ===
										pullRequest.number &&
									modifiedParams?.prNumber ===
										pullRequest.number
								) {
									reopenFilenames.add([
										originalParams,
			...
			for (const localChange of this.reviewModel.localFileChanges) {
				for (const prFileChange of reopenFilenames) {
					if (Array.isArray(prFileChange)) {
						...
						if (
							localChange.fileName === modifiedPrChange.fileName
						) {
							reopenPromises.push(
	...
	async createSuggestionFromChange(
		...
		const change = this._reviewModel.localFileChanges.find(
			(change) =>
				change.changeModel.filePath.toString() ===
				editor.uri.toString(),
		);
			...
			suggestions.map(async (suggestion) => {
				try {
					...
					vscode.window.showErrorMessage(
						vscode.l10n.t(
							"The change is outside the commenting range.",
						),
						{ modal: true },
					);
				}
	...
	async createSuggestionsFromChanges(resources: vscode.Uri[]) {
		const resourceStrings = resources.map((resource) =>
			resource.toString(),
		);
		...
		await vscode.window.withProgress(
			...
			async () => {
				await Promise.all(
					this._folderRepoManager.repository.state.workingTreeChanges.map(
						async (changeFile) => {
							if (
								!resourceStrings.includes(
									changeFile.uri.toString(),
								) ||
								changeFile.status !== Status.MODIFIED
							) {
								return;
							}
							const diff = parsePatch(
		...
		if (!hasError) {
			...
			vscode.window
				.showInformationMessage(
					vscode.l10n.t(
						"All changes have been converted to suggestions.",
					),
					{
						modal: true,
						detail: vscode.l10n.t(
							"Do you want to reset your local changes?",
						),
					},
					...
					if (response === checkoutAllFilesResponse) {
						return Promise.all(
							convertedFiles.map((changeFile) =>
								this._folderRepoManager.repository.checkout(
									changeFile.fsPath,
								),
							),
						);
			...
			vscode.window.showWarningMessage(
				...
				{
					detail: vscode.l10n.t(
						"Some of the changes may be outside of commenting ranges.\nYour changes are still available locally.",
					),
					modal: true,
				},
			);
	...
	private async registerGitHubInMemContentProvider() {
		try {
			...
			Logger.error(
				`Failed to register in mem content provider: ${e}`,
				this.id,
			);
		}
	}
	...
	private async registerCommentController() {
		if (
			...
			const changedLocalFilesChangesDisposable:
				| vscode.Disposable
				| undefined = this._reviewModel.onDidChangeLocalFileChanges(
				async () => {
					if (
						this._folderRepoManager.activePullRequest
							?.reviewThreadsCache &&
		...
		try {
			await vscode.window.withProgress(
				...
				async (progress) => {
					...
					if (!didLocalCheckout) {
						await this._folderRepoManager.fetchAndCheckout(
							pr,
							progress,
						);
					}
			...
			if (
				this._folderRepoManager.activePullRequest?.number === pr.number
			) {
	...
	public async publishBranch(branch: Branch): Promise<Branch | undefined> {
		const potentialTargetRemotes =
			await this._folderRepoManager.getAllGitHubRemotes();
		...
		if (
			...
			const fork =
				await this._folderRepoManager.tryOfferToFork(githubRepo);
			if (!fork) {
				return;
			}
			selectedRemote = (
		...
		return new Promise<Branch | undefined>(async (resolve) => {
			...
			inputBox.prompt =
				potentialTargetRemotes.length === 1
					? vscode.l10n.t(
							`The branch '{0}' is not published yet, pick a name for the upstream branch`,
							branch.name!,
						)
			...
			const validate = async function (value: string) {
				try {
					inputBox.busy = true;
					const remoteBranch = await this._reposManager.getBranch(
						remote,
						value,
					);
					if (remoteBranch) {
			...
			inputBox.onDidAccept(async () => {
				...
				const latestBranch = await this._repository.getBranch(
					branch.name!,
				);
	...
	private async getRemote(
		...
		if (!potentialTargetRemotes.length) {
			vscode.window.showWarningMessage(
				vscode.l10n.t(
					`No GitHub remotes found. Add a remote and try again.`,
				),
			);
			return;
		}
		...
		const picks: RemoteQuickPickItem[] = potentialTargetRemotes.map(
			(remote) => {
		...
		const selected: RemoteQuickPickItem | undefined =
			await vscode.window.showQuickPick<RemoteQuickPickItem>(picks, {
				ignoreFocusOut: true,
			...
			if (postCreate === "openOverview") {
				...
				if (defaultBranch) {
					...
					if (postCreate === "checkoutDefaultBranchAndShow") {
						...
						await Promise.all([
							this._folderRepoManager.checkoutDefaultBranch(
								defaultBranch,
							),
	...
	private async clear(quitReviewMode: boolean) {
		if (quitReviewMode) {
			...
			if (activePullRequest) {
				this._activePrViewCoordinator.removePullRequest(
					activePullRequest,
				);
			}
	...
	async provideTextDocumentContent(
		...
		if (changedItems.length) {
			...
			const diffChangeTypeFilter =
				commit === changedItem.sha
					? DiffChangeType.Delete
					: DiffChangeType.Add;
			const ret = (await changedItem.changeModel.diffHunks()).map(
				(diffHunk) =>
					diffHunk.diffLines
						.filter(
							(diffLine) =>
								diffLine.type !== diffChangeTypeFilter,
						)
		...
		return reviewManagers.find((reviewManager) =>
			reviewManager._folderRepoManager.gitHubRepositories.some((repo) => {
				...
				return (
					...
					(!repository ||
						reviewManager._folderRepoManager.repository ===
							repository)
				);
			}),
		);
	...
	static getReviewManagerForFolderManager(
		...
		return reviewManagers.find(
			(reviewManager) =>
				reviewManager._folderRepoManager === folderManager,
		);
	}
}
