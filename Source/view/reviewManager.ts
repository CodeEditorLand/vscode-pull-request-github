...
export class ReviewManager extends Disposable {
	...
	async createSuggestionsFromChanges(resources: vscode.Uri[]) {
		...
		const convertOneSmallHunk = async (
			changeFile: Change,
			hunk: DiffHunk,
		) => {
			try {
		...
		await vscode.window.withProgress(
			...
			async () => {
				...
				for (
					...
					i <
						this._folderRepoManager.repository.state
							.workingTreeChanges.length;
					i++
				) {
					const changeFile =
						this._folderRepoManager.repository.state
							.workingTreeChanges[i];
				...
				for (
					;
					i <
					this._folderRepoManager.repository.state.workingTreeChanges
						.length;
					i++
				) {
					const changeFile =
