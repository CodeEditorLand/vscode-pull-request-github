...
export class RepositoryChangesNode
	...
	implements vscode.TreeItem
{
	...
	constructor(
		...
		super(
			...
			pullRequest.title,
			pullRequest,
			_pullRequestManager.repository,
			_pullRequestManager,
		);
		...
		this._register(
			vscode.window.onDidChangeActiveTextEditor((e) => {
				if (
					vscode.workspace.getConfiguration(EXPLORER).get(AUTO_REVEAL)
				) {
					const tabInput =
					...
					if (tabInput instanceof vscode.TabInputTextDiff) {
						if (
							tabInput.original.scheme === Schemes.Review &&
		...
		this._register(
			this.pullRequestModel.onDidInvalidate(() => {
				this.refresh();
			}),
		);
	}

	private revealActiveEditorInTree(
		activeEditorUri: string | undefined,
	): void {
		if (this.parent.view.visible && activeEditorUri) {
			const matchingFile = this._reviewModel.localFileChanges.find(
				(change) =>
					change.changeModel.filePath.toString() === activeEditorUri,
			);
	...
	override async getTreeItem(): Promise<vscode.TreeItem> {
		...
		if (this.parent.children?.length && this.parent.children.length > 1) {
			const allSameOwner = this.parent.children.every((child) => {
				return (
					child instanceof RepositoryChangesNode &&
					child.pullRequestModel.remote.owner ===
						this.pullRequestModel.remote.owner
				);
			});
