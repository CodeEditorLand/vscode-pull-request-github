...
} from "../../github/conflictResolutionModel";

interface ConflictNode {
	conflict: Conflict;
}

export class ConflictResolutionTreeView
	implements vscode.TreeDataProvider<ConflictNode>
{
	...
	private readonly _onDidChangeTreeData: vscode.EventEmitter<
		void | ConflictNode[]
	> = new vscode.EventEmitter<void | ConflictNode[]>();
	...
	async getTreeItem(element: ConflictNode): Promise<vscode.TreeItem> {
		const resource = vscode.Uri.from({
			path: element.conflict.prHeadFilePath,
			scheme: "conflictResolution",
		});
		const item = new vscode.TreeItem(resource);
		if (
			this._conflictResolutionModel.isResolved(
				element.conflict.prHeadFilePath,
			)
		) {
			...
			item.command = {
				...
				arguments: [
					...
					this._conflictResolutionModel.mergeOutputUri(
						element.conflict,
					),
	...
	async getChildren(
		element?: ConflictNode | undefined,
		...
		if (!this._conflictResolutionModel.isResolvable()) {
			exit.appendMarkdown(
				vscode.l10n.t(
					"Not all conflicts can be resolved here. Check out the pull request to manually resolve conflicts.\n\n[Exit conflict resolution mode](command:pr.exitConflictResolutionMode)",
				),
			);
		} else {
			if (this._conflictResolutionModel.areAllConflictsResolved) {
				...
				exit.appendMarkdown(
					vscode.l10n.t(
						"Resolve all conflicts or [exit conflict resolution mode](command:pr.exitConflictResolutionMode)",
					),
				);
			}
			children = Array.from(
