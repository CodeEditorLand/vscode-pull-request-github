...
import {
	CategoryTreeNode,
	PRCategoryActionNode,
	PRCategoryActionType,
} from "./treeNodes/categoryNode";
...
export class PullRequestsTreeDataProvider
	extends Disposable
	implements vscode.TreeDataProvider<TreeNode>, BaseTreeNode
{
	...
	constructor(
		...
		this._register(
			this.prsTreeModel.onDidChangeData((folderManager) =>
				folderManager
					? this.refreshRepo(folderManager)
					: this.refresh(),
			),
		);
		...
		this._view = this._register(
			vscode.window.createTreeView("pr:github", {
			...
			vscode.workspace.onDidChangeConfiguration((e) => {
				if (
					e.affectsConfiguration(
						`${PR_SETTINGS_NAMESPACE}.${FILE_LIST_LAYOUT}`,
					)
				) {
		...
		this._register(
			this._view.onDidChangeCheckboxState(
				TreeUtils.processCheckboxUpdates,
			),
		);

		this._register(
			this._view.onDidExpandElement((expanded) => {
	...
	initialize(reviewModels: ReviewModel[], credentialStore: CredentialStore) {
		...
		for (const model of reviewModels) {
			this._register(
				model.onDidChangeLocalFileChanges((_) => {
					this.refresh();
				}),
			);
		}

		this.notificationProvider = this._register(
			new NotificationProvider(this, credentialStore, this._reposManager),
		);
	...
	private refreshRepo(manager: FolderRepositoryManager): void {
		...
		if (this._children[0] instanceof WorkspaceFolderNode) {
			...
			const node = children.find(
				(node) => node.folderManager === manager,
			);
	...
	private async needsRemotes() {
		if (
			this._reposManager?.state === ReposManagerState.NeedsAuthentication
		) {
			return [];
		}
		...
		if (remotesSetting) {
			actions = [
				new PRCategoryActionNode(
					this,
					PRCategoryActionType.NoMatchingRemotes,
				),
				new PRCategoryActionNode(
					this,
					PRCategoryActionType.ConfigureRemotes,
				),
			];
		} else {
			actions = [
				new PRCategoryActionNode(this, PRCategoryActionType.NoRemotes),
			];
		}

		const { enterpriseRemotes } = this._reposManager
			? await findDotComAndEnterpriseRemotes(
		...
		if (
			...
			!this._reposManager?.credentialStore.isAuthenticated(
			...
			actions.push(
				new PRCategoryActionNode(
					this,
					PRCategoryActionType.LoginEnterprise,
				),
			);
		}
	...
	async cachedChildren(
		element?: WorkspaceFolderNode | CategoryTreeNode,
	): Promise<TreeNode[]> {
		if (!element) {
			return this._children;
		}
	...
	async getChildren(element?: TreeNode): Promise<TreeNode[]> {
		...
		const remotes = await Promise.all(
			this._reposManager.folderManagers.map((manager) =>
				manager.getGitHubRemotes(),
			),
		);

		if (
			this._reposManager.folderManagers.filter(
				(_manager, index) => remotes[index].length > 0,
			).length === 0
		) {
		...
		if (
			this._reposManager.folderManagers.filter(
				(manager) => manager.repository.state.remotes.length > 0,
			).length === 0
		) {
			return Promise.resolve([
				new PRCategoryActionNode(this, PRCategoryActionType.Empty),
			]);
		}
