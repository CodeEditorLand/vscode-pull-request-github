...
export class CommentDecorationProvider extends TreeDecorationProvider {
	...
	registerPullRequestPropertyChangedListeners(
		...
		return model.onDidChangeReviewThreads((changed) => {
			[...changed.added, ...changed.removed].forEach((change) => {
				this._handlePullRequestPropertyChange(
					folderManager,
					model,
					change,
				);
	...
	provideFileDecoration(
		...
		if (
			uri.scheme !== Schemes.Pr &&
			uri.scheme !== Schemes.File &&
			uri.scheme !== Schemes.Review &&
			uri.scheme !== Schemes.FileChange
		) {
			return undefined;
		}
		...
		if (query && folderManager) {
			const hasComment = folderManager.gitHubRepositories.find((repo) => {
				...
				if (
					pr?.reviewThreadsCache.find(
						(c) => c.path === query.fileName,
					)
				) {
					return true;
				}
