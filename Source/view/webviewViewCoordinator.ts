...
export class WebviewViewCoordinator extends Disposable {
	...
	private create(
		...
		this._webviewViewProvider = addDisposable(
			new PullRequestViewProvider(
	...
	public show(pullRequestModel: PullRequestModel) {
		if (
			this._webviewViewProvider &&
			this._pullRequestModel.size > 0 &&
			Array.from(this._pullRequestModel.keys())[0] === pullRequestModel
		) {
